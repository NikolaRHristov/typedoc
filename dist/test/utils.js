...
function query(project, name) {
	...
	(0, assert_1.ok)(
		reflection instanceof __1.DeclarationReflection,
		`Failed to find ${name}`,
	);
	return reflection;
}
...
function getComment(project, name) {
	return __1.Comment.combineDisplayParts(
		query(project, name).comment?.summary,
	);
}
...
function equalKind(refl, kind) {
	(0, strict_1.equal)(
		refl.kind,
		kind,
		`Expected ${__1.ReflectionKind[kind]} but got ${__1.ReflectionKind[refl.kind]}`,
	);
}

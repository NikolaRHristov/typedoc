	...
	describe("guessSourceUrlTemplate helper", () => {
		it("handles a personal GitHub HTTPS URL", () => {
			...
			(0, assert_1.deepStrictEqual)(
				(0, repository_1.guessSourceUrlTemplate)(mockRemotes),
	...
	describe("getURL", () => {
		...
		try {
			...
			it("Handles replacements", function () {
				...
				const repo = repository_1.GitRepository.tryCreateRepository(
					project.cwd,
				...
				(0, assert_1.deepStrictEqual)(
					repo.getURL(
						(0, paths_1.normalizePath)(
							(0, path_1.join)(project.cwd, "test.js"),
						),
						1,
					),
...
describe("RepositoryManager - git enabled", () => {
	...
	const manager = new repository_1.RepositoryManager(
		"",
		"revision",
		"remote",
		"link:{path}",
	...
	it("Handles the simplest case", () => {
		const root = (0, paths_1.normalizePath)(
			(0, path_1.join)(fix.cwd, "root.txt"),
		);
		...
		(0, assert_1.deepStrictEqual)(
			...
			new Set(
				[
					...
					(0, paths_1.normalizePath)((0, path_1.join)(fix.cwd, f)),
				),
			),
		);
	});
	...
	it("Handles a nested repository", () => {
		const sub = (0, paths_1.normalizePath)(
			(0, path_1.join)(fix.cwd, "sub_repo/repo.txt"),
		);
		const repo = manager.getRepository(sub);
		(0, assert_1.ok)(repo);
		(0, assert_1.deepStrictEqual)(
			repo.path,
			(0, paths_1.normalizePath)((0, path_1.join)(fix.cwd, "sub_repo")),
		);
	...
	it("Caches repositories", () => {
		...
		const rootIndirect = (0, path_1.join)(
			fix.cwd,
			"self/self/self/root.txt",
		);
		...
		(0, assert_1.deepStrictEqual)(
			...
			new Map([
				...
				[
					(0, paths_1.normalizePath)(
						(0, path_1.join)(fix.cwd, "self/self/self"),
					),
					repo,
				],
				[
					(0, paths_1.normalizePath)(
						(0, path_1.join)(fix.cwd, "sub_repo"),
					),
					subRepo,
				],
				[
					(0, paths_1.normalizePath)(
						(0, path_1.join)(fix.cwd, "ignored"),
					),
					repo,
				],
				[
					(0, paths_1.normalizePath)(
						(0, path_1.join)(fix.cwd, "subfolder"),
					),
					repo,
				],
			]),
	...
	it("Handles .gitignored paths", () => {
		...
		(0, assert_1.deepStrictEqual)(
			repo?.path,
			(0, paths_1.normalizePath)(fix.cwd),
		);

...
describe("Validation Utils", () => {
	it("Should be able to validate optional values", () => {
		(0, assert_1.ok)(
			utils_1.Validation.validate(
				utils_1.Validation.optional(String),
				null,
			),
		);
		(0, assert_1.ok)(
	...
	it("Should be able to validate an array", () => {
		...
		(0, assert_1.ok)(
			!utils_1.Validation.validate([Array, String], ["a", 1]),
		);
	});
	it("Should be able to validate with a custom function", () => {
		(0, assert_1.ok)(
			utils_1.Validation.validate(utils_1.Validation.isTagString, "@foo"),
		);
		(0, assert_1.ok)(
			!utils_1.Validation.validate(utils_1.Validation.isTagString, true),
		);
	});
	...
	it("Should be able to validate an object", () => {
		...
		(0, assert_1.ok)(
			utils_1.Validation.validate(schema, { x: "", y: 0, z: 123 }),
		);
	...
	it("Should support checking for excess properties", () => {
		...
		(0, assert_1.ok)(utils_1.Validation.validate(schema, { x: "" }));
		(0, assert_1.ok)(
			!utils_1.Validation.validate(schema, { x: "", y: "" }),
		);
	});
});

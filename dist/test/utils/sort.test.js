...
describe("Sort", () => {
	...
	it("Should sort by name", () => {
		const arr = [
			new models_1.DeclarationReflection(
				"a",
	...
	it("source-order should do nothing if no symbols are available", () => {
		const proj = new models_1.ProjectReflection(
			"",
			new FileRegistry_1.FileRegistry(),
		);
		const arr = [
			new models_1.DeclarationReflection(
				"b",
	...
	it("source-order should sort by file, then by position in file", () => {
		...
		const a = new models_1.DeclarationReflection(
			"a",
			models_1.ReflectionKind.Variable,
			proj,
		);
		proj.registerSymbolId(a, aId);
		const b = new models_1.DeclarationReflection(
			"b",
			models_1.ReflectionKind.Variable,
			proj,
		);
		proj.registerSymbolId(b, bId);
		const c = new models_1.DeclarationReflection(
			"c",
			models_1.ReflectionKind.Variable,
			proj,
		);
	...
	it("enum-member-source-order should do nothing if not an enum member", () => {
		...
		proj.registerSymbolId(c, cId);
		const d = new models_1.DeclarationReflection(
			"d",
			models_1.ReflectionKind.Variable,
			proj,
		);

...
describe("Comment.combineDisplayParts", () => {
	it("Handles text and code", () => {
		...
		(0, assert_1.deepStrictEqual)(
			index_1.Comment.combineDisplayParts(parts),
			"a`b`",
		);
	});
	it("Handles inline tags", () => {
		const parts = [{ kind: "inline-tag", text: "`b`", tag: "@test" }];
		(0, assert_1.deepStrictEqual)(
			index_1.Comment.combineDisplayParts(parts),
			"{@test `b`}",
		);
...
describe("Comment.splitPartsToHeaderAndBody", () => {
	it("Handles a simple case", () => {
		...
		(0, assert_1.deepStrictEqual)(
			index_1.Comment.splitPartsToHeaderAndBody(parts),
			{
				header: "a",
	...
	it("Handles empty body", () => {
		const parts = [{ kind: "text", text: "Header\n" }];
		(0, assert_1.deepStrictEqual)(
			index_1.Comment.splitPartsToHeaderAndBody(parts),
			{
				header: "Header",

...
describe("Declaration References", () => {
	describe("String parsing", () => {
		const parse = (s) =>
			(0, declarationReference_1.parseString)(s, 0, s.length)?.[0];
		...
		it("Bails on bad escapes", () => {
			...
			(0, assert_1.deepStrictEqual)(
				parse('"\\u{FFFFFFFFFFFFFFFFFFFFFFF}"'),
				undefined,
			);
		});
		it("Parses successfully", () => {
			(0, assert_1.deepStrictEqual)(
				parse('"abc\\x41\\u0041\\u{42}z\\n\\a\\0"'),
				"abcAABz\na\0",
			);
		});
	});
	describe("Component parsing", () => {
		const parse = (s) =>
			(0, declarationReference_1.parseComponent)(s, 0, s.length)?.[0];
	...
	describe("Component Path parsing", () => {
		const parse = (s) =>
			(0, declarationReference_1.parseComponentPath)(s, 0, s.length)?.[0];
		it("Fails if it is an invalid string", () => {
			(0, assert_1.deepStrictEqual)(parse('"asdf'), undefined);
		});
	...
	describe("Meaning parsing", () => {
		const parse = (s) => {
			const meaning = (0, declarationReference_1.parseMeaning)(
				s,
				0,
				s.length,
			);
		...
		it("Parses a keyword with index", () => {
			(0, assert_1.deepStrictEqual)(parse(":class(123)"), {
				keyword: "class",
				index: 123,
			});
		});
		it("Does not parse index if invalid", () => {
			...
			(0, assert_1.deepStrictEqual)(meaning, [
				{ keyword: "class" },
				":class".length,
			]);
		});
		it("Parses an index", () => {
			(0, assert_1.deepStrictEqual)(parse(":(123)"), {
				keyword: undefined,
				index: 123,
			});
		});
		...
		it("Parses a user identifier", () => {
			(0, assert_1.deepStrictEqual)(parse(":USER_IDENT"), {
				label: "USER_IDENT",
			});
		});
	});
	describe("Symbol reference parsing", () => {
		const parse = (s) =>
			(0, declarationReference_1.parseSymbolReference)(
				s,
				0,
				s.length,
			)?.[0];
	...
	describe("Module source parsing", () => {
		const parse = (s) =>
			(0, declarationReference_1.parseModuleSource)(s, 0, s.length)?.[0];
		it("Fails if empty", () => {
			(0, assert_1.deepStrictEqual)(parse(""), undefined);
			(0, assert_1.deepStrictEqual)(parse("!"), undefined);
		});
	...
	describe("Full reference parsing", () => {
		const parse = (s) =>
			(0, declarationReference_1.parseDeclarationReference)(
				s,
				0,
				s.length,
			)?.[0];

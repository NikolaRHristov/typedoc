...
describe("Behavior Tests", () => {
	...
	it("Handles 'as const' style enums", () => {
		...
		const SomeEnumLikeTagged = (0, utils_2.query)(
			project,
			"SomeEnumLikeTagged",
		);
		(0, assert_1.deepStrictEqual)(
		...
		const ManualWithoutHelper = (0, utils_2.query)(
			project,
			"ManualEnumHelper",
		);
		(0, assert_1.deepStrictEqual)(
		...
		(0, assert_1.deepStrictEqual)(
			WithoutReadonly.kind,
			models_1.ReflectionKind.Enum,
			"WithoutReadonly",
		);
		const SomeEnumLikeNumeric = (0, utils_2.query)(
			project,
			"SomeEnumLikeNumeric",
		);
		(0, assert_1.deepStrictEqual)(
		...
		const SomeEnumLikeTaggedNumeric = (0, utils_2.query)(
			project,
		...
		const ManualEnumNumeric = (0, utils_2.query)(
			project,
			"ManualEnumNumeric",
		);
		(0, assert_1.deepStrictEqual)(
			ManualEnumNumeric.kind,
			models_1.ReflectionKind.Enum,
			"ManualEnumNumeric",
		);
		const ManualWithoutHelperNumeric = (0, utils_2.query)(
			project,
		...
		const WithoutReadonlyNumeric = (0, utils_2.query)(
			project,
		...
		const WithInvalidTypeUnionMember = (0, utils_2.query)(
			project,
		...
		const WithNumericExpression = (0, utils_2.query)(
			project,
	...
	it("Handles non-jsdoc block comments", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(a.comment?.summary),
			"jsdoc block",
		);
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(b.comment?.summary),
			"block, but not jsdoc",
		);
	});
	it("Handles const variable namespace", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(someNs.comment?.summary),
			"ns doc",
		);
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				b.signatures?.[0].comment?.summary,
			),
			"b doc",
		);
	});
	it("Should allow the user to mark a variable or function as a class with @class", () => {
		...
		(0, assert_1.deepStrictEqual)(
			...
			[
				[
					"constructor",
					models_1.ReflectionKind.singularString(
		...
		(0, assert_1.deepStrictEqual)(
			(0, utils_2.query)(project, "CallableClass.stat").flags.isStatic,
			true,
		);
		(0, assert_1.deepStrictEqual)(
			["VariableClass", "VariableClass.stat", "VariableClass.inst"].map(
				(name) => (0, utils_2.getComment)(project, name),
			),
	...
	it("Handles comments on export declarations", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(abc.comment?.summary),
			"abc",
		);
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(abcRef.comment?.summary),
			"export abc",
		);
		const foo = (0, utils_2.query)(project, "foo");
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(foo.comment?.summary),
			"export foo",
		);
	});
	...
	it("Handles @group tag", () => {
		...
		(0, assert_1.deepStrictEqual)(
			project.groups.map((g) =>
				models_1.Comment.combineDisplayParts(g.description),
			),
	...
	it("Inherits @group tag if comment is not redefined", () => {
		...
		(0, assert_1.deepStrictEqual)(
			...
			[
				[(0, utils_2.query)(project, "Cls.constructor")],
				[(0, utils_2.query)(project, "Cls.prop")],
			],
		);
	});
	...
	it("Handles simple @inheritDoc cases", () => {
		...
		const comment = new models_1.Comment(
			...
			[
				new models_1.CommentTag("@remarks", [
					{ kind: "text", text: "Remarks" },
				]),
			],
		);
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(prop.comment?.summary),
		...
		const methodComment = new models_1.Comment(
			[{ kind: "text", text: "Method description" }],
			[example],
		);
		(0, assert_1.deepStrictEqual)(
			meth.signatures?.[0].comment,
			methodComment,
		);
	});
	it("Handles more complicated @inheritDoc cases", () => {
		...
		const fooMemberComment = (0, utils_2.query)(project, "Foo.member")
			.signatures?.[0].comment;
		const xComment = (0, utils_2.query)(project, "Foo.member")
			.signatures?.[0].parameters?.[0].comment;
		...
		for (const name of ["Bar", "Baz"]) {
			(0, assert_1.deepStrictEqual)(
				(0, utils_2.query)(project, name).comment,
				fooComment,
				name,
			);
		}
	...
	it("Handles recursive @inheritDoc requests", () => {
		...
		const a = (0, utils_2.query)(project, "A");
		(0, assert_1.deepStrictEqual)(
			a.comment?.getTag("@inheritDoc")?.name,
			"B",
		);
		const b = (0, utils_2.query)(project, "B");
		(0, assert_1.deepStrictEqual)(
			b.comment?.getTag("@inheritDoc")?.name,
			"C",
		);
		const c = (0, utils_2.query)(project, "C");
		(0, assert_1.deepStrictEqual)(
			c.comment?.getTag("@inheritDoc")?.name,
			"A",
		);
		logger.expectMessage(
	...
	it("Handles @inheritDoc on signatures", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				test1.signatures[0].comment?.summary,
			),
			"A",
		);
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				test1.signatures[1].comment?.summary,
			),
			"B",
		);
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				test2.signatures?.[0].comment?.summary,
			),
			"C",
		);
	});
	it("Handles @inheritDocs which produce warnings", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(target1.comment?.summary),
			"Source",
		);
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(target2.comment?.summary),
			"Source",
		);
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
	...
	it("Handles line comments", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(c.comment?.summary),
			"",
		);
	});
	it("Handles declaration reference link resolution", () => {
		...
		for (const [refl, target] of [
			...
			(0, assert_1.deepStrictEqual)(
				getLinks((0, utils_2.query)(project, refl)).map((x) => x[1]),
				[(0, utils_2.query)(project, target).getFullName()],
			);
		}
		...
		(0, assert_1.deepStrictEqual)(
			getLinks((0, utils_2.query)(project, "URLS")),
			["https://example.com", "ftp://example.com"],
		);
		(0, assert_1.deepStrictEqual)(
			getLinks((0, utils_2.query)(project, "Globals.A")).map((x) => x[1]),
			["URLS", "A", "Globals.A"],
		);
		(0, assert_1.deepStrictEqual)(
			getLinks((0, utils_2.query)(project, "Navigation")),
			[
		...
		const foo = (0, utils_2.query)(project, "Navigation.Child.foo")
			.signatures[0];
		(0, assert_1.deepStrictEqual)(getLinks(foo), [
			[models_1.ReflectionKind.Method, "Navigation.Child.foo"],
		]);
	});
	it("Handles TypeScript based link resolution", () => {
		...
		(0, assert_1.deepStrictEqual)(getLinkTexts(localSymbolRef), [
			"A!",
			"A2!",
			"AnotherName",
		]);
		(0, assert_1.deepStrictEqual)(
			getLinks((0, utils_2.query)(project, "scoped")),
			[[models_1.ReflectionKind.Property, "Meanings.B.prop"]],
		);
		(0, assert_1.deepStrictEqual)(
			getLinkTexts((0, utils_2.query)(project, "scoped")),
			["p"],
		);
	});
	...
	it("Handles named tuple declarations", () => {
		...
		(0, assert_1.deepStrictEqual)(
			(0, utils_2.query)(project, "PartiallyNamedTuple").type?.toString(),
			"[name: string, number]",
		);
		(0, assert_1.deepStrictEqual)(
			(0, utils_2.query)(
				project,
				"PartiallyNamedTuple2",
			).type?.toString(),
			"[name?: string, number?]",
		);
		(0, assert_1.deepStrictEqual)(
	...
	it("Handles overloads", () => {
		...
		const fooComments = foo.signatures?.map((sig) =>
			models_1.Comment.combineDisplayParts(sig.comment?.summary),
		);
		(0, assert_1.deepStrictEqual)(fooComments, [
			"No arg comment\n",
			"No arg comment\n",
		]);
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(bar.comment?.summary),
	...
	it("Handles @overload tags", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				first.parameters[0].comment?.summary,
			),
			"first docs",
		);
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				second.parameters[0].comment?.summary,
			),
			"second docs",
		);
	});
	...
	it("Handles @see tags", () => {
		...
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				foo.comment?.getTag("@see")?.content,
			),
			" - Double tag\n - Second tag\n",
		);
		const bar = (0, utils_2.query)(project, "bar");
		(0, assert_1.deepStrictEqual)(
			models_1.Comment.combineDisplayParts(
				bar.comment?.getTag("@see")?.content,
			),
			"Single tag",
		);
	});
	it("Handles type aliases marked with @interface", () => {
		...
		const comments = [bar, bar.children[0], bar.children[1]].map((r) =>
			models_1.Comment.combineDisplayParts(r.comment?.summary),
		);
		(0, assert_1.deepStrictEqual)(comments, [
			"Bar docs",
			"Bar.a docs",
			"Foo.b docs",
		]);
	...
	it("Respects resolution-mode when resolving types", () => {
		...
		const MergedType = (0, utils_2.query)(
			convert("resolutionMode"),
			"MergedType",
		);
		(0, assert_1.deepStrictEqual)(
	...
	it("Special cases some `this` type occurrences", () => {
		...
		(0, assert_1.deepStrictEqual)(
			(0, utils_2.query)(project, "ThisClass.prop").type?.toString(),
			"ThisClass",
		); // Not special cased
		(0, assert_1.deepStrictEqual)(
			(0, utils_2.querySig)(
				project,
				"ThisClass.returnThisImplicit",
			).type?.toString(),
			"ThisClass",
		); // Not special cased
		(0, assert_1.deepStrictEqual)(
	...
	it("Handles renaming of destructured parameters via @param tag name inference", () => {
		...
		const params = (name) =>
			(0, utils_2.querySig)(project, name).parameters?.map((p) => p.name);
		(0, assert_1.deepStrictEqual)(
			params("functionWithADestructuredParameter"),
			["destructuredParam"],
		);
		(0, assert_1.deepStrictEqual)(
			params("functionWithADestructuredParameterAndExtraParameters"),
			["__namedParameters", "extraParameter"],
		);
		(0, assert_1.deepStrictEqual)(
			params(
	...
	it("Should not warn about recursive types", () => {
		...
		(0, assert_1.deepStrictEqual)(
			(0, utils_2.querySig)(project, "Map.getFilter").type?.toString(),
	...
	it("Handles NoInfer intrinsic type", () => {
		...
		(0, assert_1.deepStrictEqual)(
			sig.parameters[0].type?.toString(),
			"C[]",
		);
		(0, assert_1.deepStrictEqual)(
			sig.parameters[1].type?.toString(),
			"NoInfer<C>",
		);
	});
	...
	it("Cascades specified modifier tags to child reflections, #2056", () => {
		...
		const mods = (s) =>
			(0, utils_2.query)(project, s).comment?.modifierTags;
		const sigMods = (s) =>
			(0, utils_2.querySig)(project, s).comment?.modifierTags;
		(0, assert_1.deepStrictEqual)(mods("BetaStuff"), new Set(["@beta"]));
		(0, assert_1.deepStrictEqual)(
			mods("BetaStuff.AlsoBeta"),
			new Set(["@beta"]),
		);
		(0, assert_1.deepStrictEqual)(
			mods("BetaStuff.AlsoBeta.betaFish"),
			new Set(),
		);
		(0, assert_1.deepStrictEqual)(
			mods("BetaStuff.AlsoBeta.alphaFish"),
			new Set(),
		);
		(0, assert_1.deepStrictEqual)(
			sigMods("BetaStuff.AlsoBeta.betaFish"),
			new Set(["@beta"]),
		);
		(0, assert_1.deepStrictEqual)(
			sigMods("BetaStuff.AlsoBeta.alphaFish"),
			new Set(["@alpha"]),
		);
		logger.expectMessage(

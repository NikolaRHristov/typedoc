...
describe("Block Comment Lexer", () => {
	...
	it("Should handle an indented comment with stars", () => {
		const tokens = lex(`/**
            ...
            */`);
		(0, assert_1.deepStrictEqual)(tokens, [
			{ kind: lexer_1.TokenSyntaxKind.Text, text: "Text", pos: 18 },
		]);
	});
	it("Should handle an indented comment without stars", () => {
		const tokens = lex(`/*
            ...
            */`);
		(0, assert_1.deepStrictEqual)(tokens, [
			{ kind: lexer_1.TokenSyntaxKind.Text, text: "Text", pos: 15 },
		]);
	});
	...
	it("Should handle type annotations after tags at the start of a line", () => {
		...
		(0, assert_1.deepStrictEqual)(tokens, [
			...
			{
				kind: lexer_1.TokenSyntaxKind.TypeAnnotation,
				text: "{string}",
				pos: 14,
			},
...
describe("Line Comment Lexer", () => {
	function lex(text) {
		return Array.from(
			(0, lineLexer_1.lexLineComments)(text, [
				{
...
describe("Comment Parser", () => {
	...
	it("Should recognize @defaultValue as code", () => {
		...
		const comment = (0, parser_1.parseComment)(
			content,
			config,
			new minimalSourceFile_1.MinimalSourceFile(file, "<memory>"),
			logger,
			files,
		);
		(0, assert_1.deepStrictEqual)(
			comment,
			new models_1.Comment(
				[],
				[
	...
	it("Should rewrite @inheritdoc to @inheritDoc", () => {
		...
		(0, assert_1.deepStrictEqual)(
			...
			new models_1.Comment(
				[],
				[new models_1.CommentTag("@inheritDoc", [])],
			),
		);
	});
	let files;
	function getComment(text) {
		...
		const comment = (0, parser_1.parseComment)(
			content,
			config,
			new minimalSourceFile_1.MinimalSourceFile(text, "<memory>"),
			logger,
			files,
		);
		logger.expectNoOtherMessages();
...
describe("extractTagName", () => {
	it("Handles simple name", () => {
		(0, assert_1.deepStrictEqual)(
			(0, tagName_1.extractTagName)("T - abc"),
			{ name: "T", newText: "abc" },
		);
	...
	it("Handles a bracketed name", () => {
		...
		(0, assert_1.deepStrictEqual)(
			(0, tagName_1.extractTagName)("[  Ta ] - abc"),
			{ name: "Ta", newText: "abc" },
		);
		(0, assert_1.deepStrictEqual)(
			(0, tagName_1.extractTagName)("[Tb] abc"),
			{ name: "Tb", newText: "abc" },
		);
	});
	it("Handles a bracketed name with simple defaults", () => {
		(0, assert_1.deepStrictEqual)(
			(0, tagName_1.extractTagName)("[T = 1] - abc"),
			{ name: "T", newText: "abc" },
		);
		(0, assert_1.deepStrictEqual)(
			(0, tagName_1.extractTagName)("[  Ta = 'x'] - abc"),
			{
				name: "Ta",
				newText: "abc",
			},
	...
	it("Handles an incomplete bracketed name without crashing", () => {
		...
		(0, assert_1.deepStrictEqual)((0, tagName_1.extractTagName)("[Ta"), {
			name: "Ta",
			newText: "",
		});
	});

...
let JavascriptIndexPlugin = (() => {
	var _JavascriptIndexPlugin_searchComments_accessor_storage,
		_JavascriptIndexPlugin_searchDocuments_accessor_storage;
	...
	var JavascriptIndexPlugin = (_classThis = class extends _classSuper {
		get searchComments() {
			return __classPrivateFieldGet(
				this,
				_JavascriptIndexPlugin_searchComments_accessor_storage,
				"f",
			);
		}
		...
		async buildSearchIndex(event) {
			...
			const initialSearchResults = Object.values(
				...
				return (
					(refl instanceof models_1.DeclarationReflection ||
			...
			await (0, utils_1.writeFile)(
				jsonFileName,
				`window.searchData = "data:application/octet-stream;base64,${data.toString("base64")}";`,
			);
		}
		getCommentSearchText(reflection) {
			...
			return comments
				.flatMap((c) => {
					return [
						...c.summary,
						...c.blockTags.flatMap((t) => t.content),
					];
				})
				.map((part) => part.text)
				.join("\n");
		...
		constructor() {
			super(...arguments);
			_JavascriptIndexPlugin_searchComments_accessor_storage.set(
				this,
				__runInitializers(this, _searchComments_initializers, void 0),
			);
			_JavascriptIndexPlugin_searchDocuments_accessor_storage.set(
				this,
				(__runInitializers(this, _searchComments_extraInitializers),
				__runInitializers(this, _searchDocuments_initializers, void 0)),
			);
	...
	(() => {
		...
		_searchDocuments_decorators = [
			(0, utils_1.Option)("searchInDocuments"),
		];
		__esDecorate(
			_classThis,
			null,
			_searchComments_decorators,
			{
				kind: "accessor",
				name: "searchComments",
				static: false,
				private: false,
				access: {
					has: (obj) => "searchComments" in obj,
					get: (obj) => obj.searchComments,
					set: (obj, value) => {
						obj.searchComments = value;
					},
				},
				metadata: _metadata,
			},
			_searchComments_initializers,
			_searchComments_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_searchDocuments_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "searchDocuments" in obj,
					get: (obj) => obj.searchDocuments,
					set: (obj, value) => {
						obj.searchDocuments = value;
					},
				},
				metadata: _metadata,
			},
			_searchDocuments_initializers,
			_searchDocuments_extraInitializers,
		);
		__esDecorate(
			null,

...
function buildFilterItem(context, name, displayName, defaultValue) {
	return utils_1.JSX.createElement(
		...
		utils_1.JSX.createElement(
			...
			utils_1.JSX.createElement("input", {
				type: "checkbox",
				id: `tsd-filter-${name}`,
				name: name,
...
function sidebarLinks(context) {
	...
	const navLinks = Object.entries(
		context.options.getValue("navigationLinks"),
	);
	if (!links.length && !navLinks.length) return null;
	return utils_1.JSX.createElement(
		"nav",
		{ id: "tsd-sidebar-links", class: "tsd-navigation" },
		links.map(([label, url]) =>
			utils_1.JSX.createElement("a", { href: url }, label),
		),
		navLinks.map(([label, url]) =>
			utils_1.JSX.createElement(
				"a",
				{ href: url, class: "tsd-nav-link" },
				label,
			),
		),
	);
...
function settings(context) {
		...
		if (key.startsWith("@")) {
			...
			visibilityOptions.push(
				buildFilterItem(
					context,
					filterName,
					context.internationalization.translateTagName(key),
					defaultFilters[key],
				),
			);
		} else if (
	...
	return utils_1.JSX.createElement(
		"div",
		{ class: "tsd-navigation settings" },
		utils_1.JSX.createElement(
			"details",
			{ class: "tsd-accordion", open: false },
			utils_1.JSX.createElement(
				"summary",
		...
		utils_1.JSX.createElement(
			"a",
			{
				...
				class: (0, lib_1.classNames)({
					current: props.project === props.model,
				}),
			},
			...
			utils_1.JSX.createElement(
				"span",
				null,
				(0, lib_1.getDisplayName)(props.project),
			),
		),
		utils_1.JSX.createElement(
			"ul",
...
function buildSectionNavigation(context, headings) {
	...
	function finalizeLevel(finishedHandlingHeadings) {
		...
		if (
			levels[levels.length - 1].length === 0 &&
			finishedHandlingHeadings
		) {
			levels[levels.length - 1] = level;
			return;
		}
		const built = utils_1.JSX.createElement(
			"ul",
			null,
			level.map((l) => utils_1.JSX.createElement("li", null, l)),
		);
		...
		if (section.title) {
			sections.push(
				utils_1.JSX.createElement(
					...
					{
						open: true,
						class: "tsd-accordion tsd-page-navigation-section",
					},
					utils_1.JSX.createElement(
						"summary",
						{
							class: "tsd-accordion-summary",
							"data-key": `tsd-otp-${section.title}`,
						},
						context.icons.chevronDown(),
						section.title,
					),

		...
		(item) =>
			utils_1.JSX.createElement(
				"a",
				{
					...
					class:
						"tsd-signature-type " + (0, lib_1.getKindClass)(item),
				},
				item.name,
			),
	);
...
function includeIndentation() {
	return indentationDepth > 0
		? utils_1.JSX.createElement(
				"span",
				null,
				"\u00A0".repeat(indentationDepth * indentSize),
			)
...
const typeRenderers = {
	array(context, type) {
		return utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.elementType,
				models_1.TypeContext.arrayElement,
			),
			utils_1.JSX.createElement(
				"span",
				{ class: "tsd-signature-symbol" },
	...
	conditional(context, type) {
		...
		const parts = [
			renderType(
				context,
				type.checkType,
				models_1.TypeContext.conditionalCheck,
			),
			utils_1.JSX.createElement(
				"span",
			...
			renderType(
				context,
				type.extendsType,
				models_1.TypeContext.conditionalExtends,
			),
			...
			renderType(
				context,
				type.trueType,
				models_1.TypeContext.conditionalTrue,
			),
			utils_1.JSX.createElement("br", null),
			includeIndentation(),
			utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.falseType,
				models_1.TypeContext.conditionalFalse,
			),
		];
		indentationDepth--;
	...
	indexedAccess(context, type) {
		let indexType = renderType(
			context,
			type.indexType,
			models_1.TypeContext.indexedIndex,
		);
		if (
			type.objectType instanceof models_1.ReferenceType &&
			...
			if (childReflection) {
				indexType = utils_1.JSX.createElement(
					"a",
					{ href: context.urlTo(childReflection) },
					indexType,
				);
			}
		}
		return utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.objectType,
				models_1.TypeContext.indexedObject,
			),
			utils_1.JSX.createElement(
				"span",
				{ class: "tsd-signature-symbol" },
			...
			type.constraint &&
				utils_1.JSX.createElement(
					...
					renderType(
						context,
						type.constraint,
						models_1.TypeContext.inferredConstraint,
					),
				),
		);
	},
		...
		return (0, lib_1.join)(
			...
			(item) =>
				renderType(
					context,
					item,
					models_1.TypeContext.intersectionElement,
				),
		);
	},
	...
	mapped(context, type) {
		...
		parts.push(
			...
			renderType(
				context,
				type.parameterType,
				models_1.TypeContext.mappedParameter,
			),
		);
		if (type.nameType) {
			parts.push(
				...
				renderType(
					context,
					type.nameType,
					models_1.TypeContext.mappedName,
				),
			);
		}
		parts.push(
		...
		parts.push(
			renderType(
				context,
				type.templateType,
				models_1.TypeContext.mappedTemplate,
			),
		);
		indentationDepth--;
		return utils_1.JSX.createElement(
	...
	namedTupleMember(context, type) {
		return utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.element,
				models_1.TypeContext.tupleElement,
			),
		);
	},
	optional(context, type) {
		return utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.elementType,
				models_1.TypeContext.optionalElement,
			),
			utils_1.JSX.createElement(
				"span",
				{ class: "tsd-signature-symbol" },
			...
			!!type.targetType &&
				utils_1.JSX.createElement(
					...
					renderType(
						context,
						type.targetType,
						models_1.TypeContext.predicateTarget,
					),
				),
		);
	},
	query(context, type) {
		return utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.queryType,
				models_1.TypeContext.queryTypeTarget,
			),
		);
	},
	reference(context, type) {
		...
		if (reflection) {
			if (reflection.kindOf(models_1.ReflectionKind.TypeParameter)) {
				name = utils_1.JSX.createElement(
					"a",
					{
				...
				(0, lib_1.join)(
					...
					(item) =>
						renderType(
							context,
							item,
							models_1.TypeContext.referenceTypeArgument,
						),
				),
				utils_1.JSX.createElement(
					"span",
	...
	reflection(context, type, { topLevelLinks }) {
		...
		indentationDepth++;
		const renderName = (named) =>
			topLevelLinks
				? utils_1.JSX.createElement(
						"a",
						{
							class: (0, lib_1.getKindClass)(named),
							href: context.urlTo(named),
						},
						named.name,
					)
				: utils_1.JSX.createElement(
			...
			for (const index of type.declaration.indexSignatures) {
				members.push(
					utils_1.JSX.createElement(
						...
						utils_1.JSX.createElement(
							"span",
							{ class: (0, lib_1.getKindClass)(index) },
	...
	rest(context, type) {
		return utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.elementType,
				models_1.TypeContext.restElement,
			),
		);
	},
			...
			type.tail.map((item) =>
				utils_1.JSX.createElement(
					...
					renderType(
						context,
						item[0],
						models_1.TypeContext.templateLiteralElement,
					),
					utils_1.JSX.createElement(
						"span",
						{ class: "tsd-signature-symbol" },
	...
	typeOperator(context, type) {
		return utils_1.JSX.createElement(
			...
			renderType(
				context,
				type.target,
				models_1.TypeContext.typeOperatorTarget,
			),
		);
	},
	union(context, type) {

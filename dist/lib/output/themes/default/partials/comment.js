...
function commentSummary({ markdown }, props) {
	...
	return utils_1.JSX.createElement(
		...
		utils_1.JSX.createElement(utils_1.Raw, {
			html: markdown(props.comment.summary),
		}),
	);
}
function commentTags(context, props) {
	if (!props.comment) return;
	const beforeTags = context.hook(
		"comment.beforeTags",
		context,
		props.comment,
		props,
	);
	const afterTags = context.hook(
		"comment.afterTags",
		context,
		props.comment,
		props,
	...
	const tags = props.kindOf(models_1.ReflectionKind.SomeSignature)
		? props.comment.blockTags.filter(
				(tag) => tag.tag !== "@returns" && !tag.skipRendering,
			)
		...
		utils_1.JSX.createElement(
			...
			tags.map((item) => {
				...
				return utils_1.JSX.createElement(
					...
					utils_1.JSX.createElement(utils_1.Raw, {
						html: context.markdown(item.content),
					}),
				);
			}),
		),
	...
	if (props.comment) {
		for (const tag of props.comment.modifierTags) {
			if (!flagsNotRendered.includes(tag)) {
				allFlags.push(
					context.internationalization.translateTagName(tag),
				);
			}
		}
	}
	return (0, lib_1.join)(" ", allFlags, (item) =>
		utils_1.JSX.createElement("code", { class: "tsd-tag" }, item),
	);
}

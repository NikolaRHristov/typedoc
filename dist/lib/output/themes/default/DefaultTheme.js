...
class DefaultTheme extends theme_1.Theme {
	getRenderContext(pageEvent) {
		return new DefaultThemeRenderContext_1.DefaultThemeRenderContext(
			this,
			pageEvent,
	...
	getUrls(project) {
		...
		if (!hasReadme(this.application.options.getValue("readme"))) {
			...
			project.children?.every((child) =>
				child.kindOf(models_1.ReflectionKind.Module),
			)
		) {
	...
	render(page, template) {
		...
		return (
			"<!DOCTYPE html>" + utils_1.JSX.renderElement(templateOutput) + "\n"
		);
	}
	/**
	...
	buildNavigation(project) {
		...
		function toNavigation(element) {
			...
			if (
				element instanceof models_1.ReflectionCategory ||
			...
			return {
				...
				class: (0, lib_1.classNames)(
					...
					theme.getReflectionClasses(element),
				),
		...
		function getNavigationElements(parent) {
			...
			if (parent instanceof models_1.ReflectionGroup) {
				if (
					shouldShowCategories(parent.owningReflection, opts) &&
					parent.categories
				) {
					return (0, utils_1.filterMap)(
						parent.categories,
						toNavigation,
					);
				}
			...
			if (
				opts.includeFolders &&
				parent.childrenIncludingDocuments?.some((child) =>
					child.name.includes("/"),
				)
			) {
			...
			return (0, utils_1.filterMap)(
				parent.childrenIncludingDocuments,
				toNavigation,
			);
		}
		function filterMapWithNoneCollection(reflection) {
			const none = reflection.find(
				(x) => x.title.toLocaleLowerCase() === "none",
			);
			const others = reflection.filter(
				(x) => x.title.toLocaleLowerCase() !== "none",
			);
			const mappedOthers = (0, utils_1.filterMap)(others, toNavigation);
			if (none) {
				const noneMappedChildren = (0, utils_1.filterMap)(
					none.children,
					toNavigation,
				);
		...
		if (key === "inherited") {
			...
			if (key === "@deprecated") {
				if (reflection.isDeprecated()) {
					classes.push(
						(0, lib_1.toStyleClass)(`tsd-is-${key.substring(1)}`),
					);
				}
			} else if (
				reflection.comment?.hasModifier(key) ||

 ...
 */
let MarkedPlugin = (() => {
	var _MarkedPlugin_lightTheme_accessor_storage,
		_MarkedPlugin_darkTheme_accessor_storage,
		_MarkedPlugin_markdownItOptions_accessor_storage,
		_MarkedPlugin_markdownLinkExternal_accessor_storage;
	...
	var MarkedPlugin = (_classThis = class extends _classSuper {
		constructor() {
			super(...arguments);
			_MarkedPlugin_lightTheme_accessor_storage.set(
				this,
				__runInitializers(this, _lightTheme_initializers, void 0),
			);
			_MarkedPlugin_darkTheme_accessor_storage.set(
				this,
				(__runInitializers(this, _lightTheme_extraInitializers),
				__runInitializers(this, _darkTheme_initializers, void 0)),
			);
			_MarkedPlugin_markdownItOptions_accessor_storage.set(
				this,
				(__runInitializers(this, _darkTheme_extraInitializers),
				__runInitializers(
					this,
					_markdownItOptions_initializers,
					void 0,
				)),
			);
			_MarkedPlugin_markdownLinkExternal_accessor_storage.set(
				this,
				(__runInitializers(this, _markdownItOptions_extraInitializers),
				__runInitializers(
					this,
					_markdownLinkExternal_initializers,
					void 0,
				)),
			);
			this.parser = __runInitializers(
				this,
		...
		get lightTheme() {
			return __classPrivateFieldGet(
				this,
				_MarkedPlugin_lightTheme_accessor_storage,
				"f",
			);
		}
		...
		set markdownLinkExternal(value) {
			__classPrivateFieldSet(
				this,
				_MarkedPlugin_markdownLinkExternal_accessor_storage,
				value,
		...
		initialize() {
			super.initialize();
			this.owner.on(
				events_1.MarkdownEvent.PARSE,
				this.onParseMarkdown.bind(this),
			);
		}
		/**
				...
				switch (part.kind) {
							...
							case "@linkplain": {
								if (part.target) {
									...
									if (typeof part.target === "string") {
										...
										kindClass =
											models_1.ReflectionKind.classString(
												part.target.kind,
											);
									}
									if (useHtml) {
					...
					case "relative-link":
						switch (typeof part.target) {
							case "number": {
								const refl = page.project.files.resolve(
									part.target,
								);
								...
								const fileName = page.project.files.getName(
									part.target,
								);
								if (fileName) {
									result.push(
		...
		setupParser() {
			...
			const loader =
				this.application.options.getValue("markdownItLoader");
			loader(this.parser);
			...
			this.parser.renderer.rules["link_open"] = (
				tokens,
				idx,
				options,
				_env,
	...
	(() => {
		...
		_markdownItOptions_decorators = [
			(0, utils_1.Option)("markdownItOptions"),
		];
		_markdownLinkExternal_decorators = [
			(0, utils_1.Option)("markdownLinkExternal"),
		];
		__esDecorate(
			_classThis,
			null,
		...
		__esDecorate(
			...
			_markdownItOptions_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "markdownItOptions" in obj,
					get: (obj) => obj.markdownItOptions,
					set: (obj, value) => {
						obj.markdownItOptions = value;
					},
				},
				metadata: _metadata,
			},
			_markdownItOptions_initializers,
			_markdownItOptions_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_markdownLinkExternal_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "markdownLinkExternal" in obj,
					get: (obj) => obj.markdownLinkExternal,
					set: (obj, value) => {
						obj.markdownLinkExternal = value;
					},
				},
				metadata: _metadata,
			},

...
class DeclarationReflection extends container_1.ContainerReflection {
	...
	traverse(callback) {
		for (const parameter of this.typeParameters?.slice() || []) {
			if (
				callback(
					parameter,
					abstract_1.TraverseProperty.TypeParameter,
				) === false
			) {
				return;
			}
		...
		if (this.type instanceof types_1.ReflectionType) {
			if (
				callback(
					...
					abstract_1.TraverseProperty.TypeLiteral,
				) === false
			) {
				return;
			}
		...
		for (const signature of this.signatures?.slice() || []) {
			if (
				callback(signature, abstract_1.TraverseProperty.Signatures) ===
				false
			) {
				return;
			}
		...
		for (const signature of this.indexSignatures?.slice() || []) {
			if (
				callback(
					signature,
					abstract_1.TraverseProperty.IndexSignature,
				) === false
			) {
				return;
			}
		...
		if (this.getSignature) {
			if (
				callback(
					this.getSignature,
					abstract_1.TraverseProperty.GetSignature,
				) === false
			) {
				return;
			}
		...
		if (this.setSignature) {
			if (
				callback(
					this.setSignature,
					abstract_1.TraverseProperty.SetSignature,
				) === false
			) {
				return;
			}
	...
	toObject(serializer) {
		return {
			...
			readme: comments_1.Comment.serializeDisplayParts(
				serializer,
				this.readme,
			),
		};
	...
	fromObject(de, obj) {
		...
		if (obj.readme) {
			this.readme = comments_1.Comment.deserializeDisplayParts(
				de,
				obj.readme,
			);
		}
				...
				for (const [id, sid] of Object.entries(obj.symbolIdMap || {})) {
					...
					if (refl) {
						this.project.registerSymbolId(
							refl,
							new ReflectionSymbolId_1.ReflectionSymbolId(sid),
						);
					} else {
						de.logger.warn(

...
class ContainerReflection extends abstract_1.Reflection {
	...
	traverse(callback) {
		for (const child of this.children?.slice() || []) {
			if (
				callback(child, abstract_1.TraverseProperty.Children) === false
			) {
				return;
			}
		}
		for (const child of this.documents?.slice() || []) {
			if (
				callback(child, abstract_1.TraverseProperty.Documents) === false
			) {
				return;
			}
		}
	...
	fromObject(de, obj) {
		...
		this.groups = de.reviveMany(
			obj.groups,
			(group) => new ReflectionGroup_1.ReflectionGroup(group.title, this),
		);
		this.categories = de.reviveMany(
			obj.categories,
			(cat) => new ReflectionCategory_1.ReflectionCategory(cat.title),
		);
	}
}

...
class ReflectionSymbolId {
	constructor(symbol, declaration) {
		if ("name" in symbol) {
			...
			this.fileName = (0, paths_1.normalizePath)(
				declaration?.getSourceFile().fileName ?? "",
			);
			if (symbol.declarations?.some(typescript_1.default.isSourceFile)) {
				this.qualifiedName = "";
			} else {
				this.qualifiedName = (0, tsutils_1.getQualifiedName)(
					symbol,
					symbol.name,
				);
			}
...
function resolveDeclarationMaps(file) {
	...
	if (!(0, fs_1.existsSync)(mapFile)) return file;
	let sourceMap;
	try {
...
function addInferredDeclarationMapPaths(opts, files) {
	...
	for (const file of files) {
		const mapFile = (0, paths_1.normalizePath)(
			(0, path_1.resolve)(
				declDir,
				(0, path_1.relative)(rootDir, file),
			).replace(/\.([cm]?[tj]s)x?$/, ".d.$1"),
		);

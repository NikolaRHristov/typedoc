...
exports.UnknownType =
	exports.UnionType =
	exports.TypeOperatorType =
	exports.NamedTupleMember =
	exports.TupleType =
	exports.TemplateLiteralType =
	exports.RestType =
	exports.ReflectionType =
	exports.ReferenceType =
	exports.QueryType =
	exports.PredicateType =
	exports.OptionalType =
	exports.MappedType =
	exports.LiteralType =
	exports.IntrinsicType =
	exports.IntersectionType =
	exports.InferredType =
	exports.IndexedAccessType =
	exports.ConditionalType =
	exports.ArrayType =
	exports.TypeContext =
	exports.Type =
		void 0;
...
class ArrayType extends Type {
	...
	getTypeString() {
		return (
			this.elementType.stringify(exports.TypeContext.arrayElement) + "[]"
		);
	}
	needsParenthesis() {
		return false;
...
class MappedType extends Type {
	...
	getTypeString() {
		...
		if (this.nameType) {
			parts.push(
				" as ",
				this.nameType.stringify(exports.TypeContext.mappedName),
			);
		}
		parts.push(
			"]",
			opt,
			": ",
			this.templateType.stringify(exports.TypeContext.mappedTemplate),
			" }",
		);
		return parts.join("");
	}
...
class OptionalType extends Type {
	...
	getTypeString() {
		return (
			this.elementType.stringify(exports.TypeContext.optionalElement) +
			"?"
		);
	}
	needsParenthesis() {
...
class PredicateType extends Type {
	...
	getTypeString() {
		...
		if (this.targetType) {
			out.push(
				"is",
				this.targetType.stringify(exports.TypeContext.predicateTarget),
			);
		}
		return out.join(" ");
	}
...
class ReferenceType extends Type {
	...
	static createSymbolReference(symbol, context, name) {
		const ref = new ReferenceType(
			name ?? symbol.name,
			new ReflectionSymbolId_1.ReflectionSymbolId(symbol),
			context.project,
			(0, tsutils_1.getQualifiedName)(symbol, name ?? symbol.name),
		);
	...
	getTypeString() {
		...
		if (this.typeArguments && this.typeArguments.length > 0) {
			...
			typeArgs += this.typeArguments
				.map((arg) =>
					arg.stringify(exports.TypeContext.referenceTypeArgument),
				)
				.join(", ");
			typeArgs += ">";
		}
	...
	fromObject(de, obj) {
		...
		if (typeof obj.target === "number" && obj.target !== -1) {
			...
			this._target = new ReflectionSymbolId_1.ReflectionSymbolId(
				obj.target,
			);
		}
	...
	fromObject(de, obj) {
		if (obj.elementSummaries) {
			this.elementSummaries = obj.elementSummaries.map((parts) =>
				comments_1.Comment.deserializeDisplayParts(de, parts),
			);
		}
	}
	toObject(serializer) {
		return {
			...
			elementSummaries: this.elementSummaries?.map((parts) =>
				comments_1.Comment.serializeDisplayParts(serializer, parts),
			),
		};
	}
}

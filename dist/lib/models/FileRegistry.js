...
class FileRegistry {
	...
	register(sourcePath, relativePath) {
		return this.registerAbsolute(
			(0, path_1.resolve)((0, path_1.dirname)(sourcePath), relativePath),
		);
	}
	...
	toObject(ser) {
		...
		for (const [key, val] of this.mediaToPath.entries()) {
			result.entries[key] = (0, utils_1.normalizePath)(
				(0, path_1.relative)(ser.projectRoot, val),
			);
		}
	...
	fromObject(de, obj) {
		for (const [key, val] of Object.entries(obj.entries)) {
			const absolute = (0, utils_1.normalizePath)(
				(0, path_1.resolve)(de.projectRoot, val),
			);
...
class ValidatingFileRegistry extends FileRegistry {
	register(sourcePath, relativePath) {
		const absolute = (0, path_1.resolve)(
			(0, path_1.dirname)(sourcePath),
			relativePath,
		);

...
function createSignature(context, kind, signature, symbol, declaration) {
	(0, assert_1.default)(
		context.scope instanceof models_1.DeclarationReflection,
	);
	declaration ||= signature.getDeclaration();
	const sigRef = new models_1.SignatureReflection(
		kind == models_1.ReflectionKind.ConstructorSignature
	...
	if (symbol && declaration) {
		context.project.registerSymbolId(
			sigRef,
			new ReflectionSymbolId_1.ReflectionSymbolId(symbol, declaration),
		);
	}
	let parentReflection = context.scope;
	if (
		parentReflection.kindOf(models_1.ReflectionKind.TypeLiteral) &&
	...
	if (declaration) {
		...
		if (parentReflection.comment?.discoveryId !== sigComment?.discoveryId) {
			...
			if (
				parentReflection.kindOf(
					models_1.ReflectionKind.MayContainDocuments,
				)
			) {
...
function createConstructSignatureWithType(context, signature, classType) {
	...
	sigRef.type = models_1.ReferenceType.createResolvedReference(
...
function convertParameters(context, sigRef, parameters, parameterNodes) {
	return parameters.map((param, i) => {
		...
		const paramRefl = new models_1.ParameterReflection(
		...
		paramRefl.defaultValue = (0, convert_expression_1.convertDefaultValue)(
			parameterNodes?.[i],
		);
		...
		if (declaration) {
			isRest = typescript_1.default.isParameter(declaration)
				...
				: !!declaration.typeExpression &&
					typescript_1.default.isJSDocVariadicType(
...
function checkForDestructuredParameterDefaults(param, decl) {
	if (!decl || !typescript_1.default.isParameter(decl)) return;
	if (param.name !== "__namedParameters") return;
	if (!typescript_1.default.isObjectBindingPattern(decl.name)) return;
	...
	for (const child of param.type.declaration.children || []) {
		...
		if (tsChild) {
			child.defaultValue = (0, convert_expression_1.convertDefaultValue)(
				tsChild,
			);
		}
	}
...
function convertTypeParameters(context, parent, parameters) {
	return parameters?.map((param) => {
		...
		const paramRefl = new models_1.TypeParameterReflection(
			param.symbol.name,
			parent,
			variance,
		);
		...
		if (
			declaration?.modifiers?.some(
				(m) => m.kind === typescript_1.default.SyntaxKind.ConstKeyword,
			)
		) {
	...
	return nodes?.flatMap((node) => {
		return node.typeParameters.map((param, index) => {
			...
			if (
				param.modifiers?.some(
					(m) =>
						m.kind === typescript_1.default.SyntaxKind.ConstKeyword,
				)
			) {
				paramRefl.flags.setFlag(models_1.ReflectionFlag.Const, true);
			}
...
function getVariance(modifiers) {
	const hasIn = modifiers?.some(
		(mod) => mod.kind === typescript_1.default.SyntaxKind.InKeyword,
	);
	const hasOut = modifiers?.some(
		(mod) => mod.kind === typescript_1.default.SyntaxKind.OutKeyword,
	);
	if (hasIn && hasOut) {
...
function convertPredicate(predicate, context) {
	...
	return new models_1.PredicateType(
		name,
		asserts,
		predicate.type

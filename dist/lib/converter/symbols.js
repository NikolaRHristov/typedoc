...
for (const key of Object.keys(symbolConverters)) {
	if (!Number.isInteger(Math.log2(+key))) {
		throw new Error(
			`Symbol converter for key ${typescript_1.default.SymbolFlags[+key]} does not specify a valid flag value.`,
		);
	}
	if (!conversionOrder.includes(+key)) {
		throw new Error(
			`Symbol converter for key ${typescript_1.default.SymbolFlags[+key]} is not specified in conversionOrder`,
		);
	}
}
...
function convertSymbol(context, symbol, exportSymbol) {
	...
	let flags = (0, enum_1.removeFlag)(
		symbol.flags,
		typescript_1.default.SymbolFlags.Transient |
	...
	for (const flag of (0, enum_1.getEnumFlags)(flags & ~allConverterFlags)) {
		context.logger.verbose(
			`Missing converter for symbol: ${symbol.name} with flag ${typescript_1.default.SymbolFlags[flag]}`,
		);
	}
...
function convertEnum(context, symbol, exportSymbol) {
	...
	convertSymbols(
		...
		context.checker
			...
			.filter(
				(s) => s.flags & typescript_1.default.SymbolFlags.EnumMember,
			),
	);
}
...
function convertNamespace(context, symbol, exportSymbol) {
	...
	if (
		symbol
			...
			?.some(
				(d) =>
					typescript_1.default.isModuleDeclaration(d) ||
					typescript_1.default.isSourceFile(d),
			) !== true
	) {
...
function convertTypeAlias(context, symbol, exportSymbol) {
	const declaration = symbol
		...
		?.find(
			(d) =>
				typescript_1.default.isTypeAliasDeclaration(d) ||
	...
	if (typescript_1.default.isTypeAliasDeclaration(declaration)) {
		...
		reflection.typeParameters = declaration.typeParameters?.map((param) =>
			(0, signature_1.createTypeParamReflection)(
				param,
				context.withScope(reflection),
			),
		);
	} else if (
		typescript_1.default.isJSDocTypedefTag(declaration) ||
		typescript_1.default.isJSDocEnumTag(declaration)
	) {
		(0, jsdoc_1.convertJsDocAlias)(
			context,
			symbol,
			declaration,
			exportSymbol,
		...
		(0, jsdoc_1.convertJsDocCallback)(
			context,
			symbol,
			declaration,
			exportSymbol,
...
function attachUnionComments(context, declaration, union) {
	...
	if (list.kind !== typescript_1.default.SyntaxKind.SyntaxList) return;
	let unionIndex = 0;
...
function convertTypeAliasAsInterface(
	...
	if (declaration.typeParameters) {
		reflection.typeParameters = declaration.typeParameters.map((param) => {
			...
			(0, assert_1.default)(
				declaration &&
					typescript_1.default.isTypeParameterDeclaration(
						declaration,
					),
			);
...
function convertFunctionOrMethod(context, symbol, exportSymbol) {
	...
	if (!isMethod) {
		const comment = context.getComment(
			symbol,
	...
	const declarations =
		symbol.getDeclarations()?.filter(typescript_1.default.isFunctionLike) ??
		[];
	...
	if (
		...
		(0, enum_1.hasAllFlags)(
			typescript_1.default.getCombinedModifierFlags(declarations[0]),
			typescript_1.default.ModifierFlags.Private,
		)
	) {
		return;
	}
	const locationDeclaration =
		symbol.parent
			...
			?.find(
				(d) =>
					typescript_1.default.isClassDeclaration(d) ||
					typescript_1.default.isInterfaceDeclaration(d),
			) ??
	...
	const reflection = context.createDeclarationReflection(
		context.scope.kindOf(models_1.ReflectionKind.MethodContainer)
...
function convertClassOrInterface(context, symbol, exportSymbol) {
	const reflection = context.createDeclarationReflection(
		typescript_1.default.SymbolFlags.Class & symbol.flags
	...
	const classDeclaration = symbol
		...
		?.find(
			(d) =>
				typescript_1.default.isClassDeclaration(d) ||
				typescript_1.default.isFunctionDeclaration(d),
		);
	...
	const extendedTypes = (0, nodes_1.getHeritageTypes)(
		declarations,
	...
	const implementedTypes = (0, nodes_1.getHeritageTypes)(
		declarations,
	...
	if (instanceType.typeParameters) {
		reflection.typeParameters = instanceType.typeParameters.map((param) => {
			...
			return (0, signature_1.createTypeParamReflection)(
				declaration,
				reflectionContext,
			);
		});
	...
	context.checker
		.getSignaturesOfType(
			instanceType,
			typescript_1.default.SignatureKind.Call,
		)
		.forEach((sig) =>
			(0, signature_1.createSignature)(
				reflectionContext,
...
function convertProperty(context, symbol, exportSymbol) {
	...
	if (
		...
		declarations.every(
			(decl) =>
				typescript_1.default.isMethodSignature(decl) ||
	...
	const reflection = context.createDeclarationReflection(
		context.scope.kindOf(models_1.ReflectionKind.VariableContainer)
	...
	reflection.defaultValue =
		declaration &&
		(0, convert_expression_1.convertDefaultValue)(declaration);
	reflection.type = context.converter.convertType(
		context.withScope(reflection),
...
function convertConstructor(context, symbol) {
	...
	const declarations =
		symbol
			.getDeclarations()
			?.filter(typescript_1.default.isConstructorDeclaration) ?? [];
...
function convertConstructSignatures(context, symbol) {
	...
	const constructSignatures = context.checker.getSignaturesOfType(
		type,
		typescript_1.default.SignatureKind.Construct,
	);
	if (constructSignatures.length) {
		const constructMember = new models_1.DeclarationReflection(
			"constructor",
			models_1.ReflectionKind.Constructor,
			context.scope,
		);
...
function createAlias(target, context, symbol, exportSymbol) {
	...
	const ref = new models_1.ReferenceReflection(
...
function convertVariable(context, symbol, exportSymbol) {
	...
	setModifiers(symbol, declaration, reflection);
	reflection.defaultValue = (0, convert_expression_1.convertDefaultValue)(
		declaration,
	);
	context.finalizeDeclarationReflection(reflection);
...
function isEnumLike(checker, type, location) {
	if (
		...
		!(0, enum_1.hasAllFlags)(
			type.flags,
			typescript_1.default.TypeFlags.Object,
		)
	) {
		return false;
	}
...
function convertSymbolAsClass(context, symbol, exportSymbol) {
	...
	if (ctors.length) {
		...
		for (const sig of ctors) {
			(0, signature_1.createConstructSignatureWithType)(
				constructContext,
				sig,
				reflection,
			);
...
function convertAccessor(context, symbol, exportSymbol) {
	...
	const getDeclaration = symbol
		.getDeclarations()
		?.find(typescript_1.default.isGetAccessor);
	if (getDeclaration) {
		const signature =
	...
	const setDeclaration = symbol
		.getDeclarations()
		?.find(typescript_1.default.isSetAccessor);
	if (setDeclaration) {
		const signature =
...
function setModifiers(symbol, declaration, reflection) {
	...
	const modifiers =
		typescript_1.default.getCombinedModifierFlags(declaration);
	if (
	...
	if (
		(0, enum_1.hasAllFlags)(
			modifiers,
			typescript_1.default.ModifierFlags.Protected,
		)
	) {
	...
	if (
		(0, enum_1.hasAllFlags)(
			modifiers,
			typescript_1.default.ModifierFlags.Public,
		)
	) {
		reflection.setFlag(models_1.ReflectionFlag.Public);
	}
	reflection.setFlag(
		models_1.ReflectionFlag.Optional,
		(0, enum_1.hasAllFlags)(
			symbol.flags,
			typescript_1.default.SymbolFlags.Optional,
		),
	...
	reflection.setFlag(
		models_1.ReflectionFlag.Readonly,
		(0, enum_1.hasAllFlags)(
			typescript_1.default.getCheckFlags(symbol),
			typescript_1.default.CheckFlags.Readonly,
		) ||
			(0, enum_1.hasAllFlags)(
				modifiers,
				typescript_1.default.ModifierFlags.Readonly,
			),
	);
	reflection.setFlag(
		models_1.ReflectionFlag.Abstract,
		(0, enum_1.hasAllFlags)(
			modifiers,
			typescript_1.default.ModifierFlags.Abstract,
		),
	);
	if (
		reflection.kindOf(models_1.ReflectionKind.Variable) &&
		(0, enum_1.hasAllFlags)(
			symbol.flags,
			typescript_1.default.SymbolFlags.BlockScopedVariable,
		)

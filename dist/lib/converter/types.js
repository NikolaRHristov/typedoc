...
const conditionalConverter = {
	...
	convert(context, node) {
		return new models_1.ConditionalType(
...
const constructorConverter = {
	...
	convert(context, node) {
		...
		const reflection = new models_1.DeclarationReflection(
			"__type",
		...
		const signature = new models_1.SignatureReflection(
			"__type",
		...
		if (
			node.modifiers?.some(
				(m) =>
					m.kind === typescript_1.default.SyntaxKind.AbstractKeyword,
			)
		) {
		...
		context.project.registerSymbolId(
			signature,
			new ReflectionSymbolId_1.ReflectionSymbolId(symbol, node),
		);
		context.registerReflection(signature, void 0);
		const signatureCtx = rc.withScope(signature);
		reflection.signatures = [signature];
		signature.type = convertType(signatureCtx, node.type);
		signature.parameters = (0, signature_1.convertParameterNodes)(
			signatureCtx,
			signature,
			node.parameters,
		);
		signature.typeParameters = (0, signature_1.convertTypeParameterNodes)(
			signatureCtx,
			node.typeParameters,
		);
...
const exprWithTypeArgsConverter = {
	...
	convert(context, node) {
		...
		const ref = models_1.ReferenceType.createSymbolReference(
...
const importType = {
	...
	convert(context, node) {
		...
		(0, assert_1.default)(
			symbol,
			"Missing symbol when converting import type node",
		);
		return models_1.ReferenceType.createSymbolReference(
	...
	convertType(context, type) {
		...
		(0, assert_1.default)(
			symbol,
			"Missing symbol when converting import type",
...
const indexedAccessConverter = {
	...
	convert(context, node) {
		return new models_1.IndexedAccessType(
...
const inferredConverter = {
	...
	convert(context, node) {
		return new models_1.InferredType(
...
const intersectionConverter = {
	...
	convert(context, node) {
		return new models_1.IntersectionType(
...
const optionalConverter = {
	...
	convert(context, node) {
		return new models_1.OptionalType(
			(0, reflections_1.removeUndefined)(convertType(context, node.type)),
		);
	},
...
const typeLiteralConverter = {
	...
	convertType(context, type) {
		...
		if (symbol) {
			(0, index_signature_1.convertIndexSignatures)(
				context.withScope(reflection),
				symbol,
			);
		}
...
const queryConverter = {
	...
	convert(context, node) {
		...
		if (!querySymbol) {
			...
			return new models_1.QueryType(
				models_1.ReferenceType.createBrokenReference(
	...
	convertType(context, type, node) {
		...
		(0, assert_1.default)(
			symbol,
			`Query type failed to get a symbol for: ${context.checker.typeToString(type)}. This is a bug.`,
		);
		const ref = models_1.ReferenceType.createSymbolReference(
			context.resolveAliasedSymbol(symbol),
			context,
...
const referenceConverter = {
	...
	convert(context, node) {
		...
		if (isArray) {
			return new models_1.ArrayType(
				convertType(context, node.typeArguments?.[0]),
			);
		}
		const symbol = context.expectSymbolAtLocation(node.typeName);
		const name = node.typeName.getText();
		const type = models_1.ReferenceType.createSymbolReference(
			context.resolveAliasedSymbol(symbol),
			context,
			name,
		);
	...
	convertType(context, type, node) {
		...
		if (!symbol) {
			...
			const ref = models_1.ReferenceType.createBrokenReference(
		...
		if (type.flags & typescript_1.default.TypeFlags.Substitution) {
			// NoInfer<T>
			ref.typeArguments = [convertType(context, type.baseType)];
		} else if (type.flags & typescript_1.default.TypeFlags.StringMapping) {
			ref.typeArguments = [convertType(context, type.type)];
		} else {
			ref.typeArguments = (
...
const namedTupleMemberConverter = {
	...
	convert(context, node) {
		...
		return new models_1.NamedTupleMember(
			node.name.getText(),
			!!node.questionToken,
			innerType,
		);
...
const mappedConverter = {
	...
	convert(context, node) {
		...
		return new models_1.MappedType(
			node.typeParameter.name.text,
	...
	convert(context, node) {
		switch (node.literal.kind) {
			...
			case typescript_1.default.SyntaxKind.FalseKeyword:
				return new models_1.LiteralType(
					node.literal.kind ===
...
const templateLiteralConverter = {
	...
	convert(context, node) {
		return new models_1.TemplateLiteralType(
			node.head.text,
	...
	convertType(context, type) {
		...
		const parts = [];
		for (const [a, b] of (0, array_1.zip)(
			type.types,
			type.texts.slice(1),
		)) {
	...
	convertType(context, type, node) {
		...
		if (type.target.labeledElementDeclarations) {
			...
			elements = elements?.map((el, i) => {
				...
				return namedDecl
					? new models_1.NamedTupleMember(
							namedDecl.name.getText(),
							!!namedDecl.questionToken,
							(0, reflections_1.removeUndefined)(el),
						)
					: el;
			});
		}
		elements = elements?.map((el, i) => {
			if (
				type.target.elementFlags[i] &
				typescript_1.default.ElementFlags.Variable
			) {
				// In the node case, we don't need to add the wrapping Array type... but we do here.
				if (el instanceof models_1.NamedTupleMember) {
					return new models_1.RestType(
						new models_1.NamedTupleMember(
							el.name,
							el.isOptional,
							new models_1.ArrayType(el.element),
						),
					);
				}
			...
			if (
				type.target.elementFlags[i] &
					typescript_1.default.ElementFlags.Optional &&
...
const typeOperatorConverter = {
	...
	convert(context, node) {
		return new models_1.TypeOperatorType(
	...
	convertType(context, type, node) {
		...
		if (node.operator === typescript_1.default.SyntaxKind.ReadonlyKeyword) {
			...
			const inner =
				resolved.objectFlags & typescript_1.default.ObjectFlags.Tuple
...
function resolveReference(type) {
	if (
		(0, nodes_1.isObjectType)(type) &&
		type.objectFlags & typescript_1.default.ObjectFlags.Reference
	) {
		return type.target;
	}
	return type;
...
function normalizeUnion(types) {
	...
	if (trueIndex !== -1 && falseIndex !== -1) {
		...
		types.splice(
			...
			1,
			new models_1.IntrinsicType("boolean"),
		);
	}
}

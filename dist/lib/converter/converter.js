...
let Converter = (() => {
	var _Converter_externalPattern_accessor_storage,
		_Converter_excludeExternals_accessor_storage,
		_Converter_excludeNotDocumented_accessor_storage,
		_Converter_excludePrivate_accessor_storage,
		_Converter_excludeProtected_accessor_storage,
		_Converter_excludeReferences_accessor_storage,
		_Converter_commentStyle_accessor_storage,
		_Converter_validation_accessor_storage,
		_Converter_externalSymbolLinkMappings_accessor_storage,
		_Converter_preserveLinkText_accessor_storage,
		_Converter_maxTypeConversionDepth_accessor_storage;
	...
	var Converter = (_classThis = class extends _classSuper {
		...
		get externalPattern() {
			return __classPrivateFieldGet(
				this,
				_Converter_externalPattern_accessor_storage,
				"f",
			);
		}
		...
		/** @internal */
		get externalSymbolLinkMappings() {
			return __classPrivateFieldGet(
				this,
				_Converter_externalSymbolLinkMappings_accessor_storage,
				"f",
		...
		constructor(owner) {
			super(owner);
			_Converter_externalPattern_accessor_storage.set(
				this,
				__runInitializers(this, _externalPattern_initializers, void 0),
			);
			this.externalPatternCache = __runInitializers(
				this,
				_externalPattern_extraInitializers,
			);
			_Converter_excludeExternals_accessor_storage.set(
				this,
				__runInitializers(this, _excludeExternals_initializers, void 0),
			);
			_Converter_excludeNotDocumented_accessor_storage.set(
				this,
				(__runInitializers(this, _excludeExternals_extraInitializers),
				__runInitializers(
					this,
					_excludeNotDocumented_initializers,
					void 0,
				)),
			);
			_Converter_excludePrivate_accessor_storage.set(
				this,
				(__runInitializers(
					this,
					_excludeNotDocumented_extraInitializers,
				),
				__runInitializers(this, _excludePrivate_initializers, void 0)),
			);
			_Converter_excludeProtected_accessor_storage.set(
				this,
				(__runInitializers(this, _excludePrivate_extraInitializers),
				__runInitializers(
					this,
					_excludeProtected_initializers,
					void 0,
				)),
			);
			_Converter_excludeReferences_accessor_storage.set(
				this,
				(__runInitializers(this, _excludeProtected_extraInitializers),
				__runInitializers(
					this,
					_excludeReferences_initializers,
					void 0,
				)),
			);
			_Converter_commentStyle_accessor_storage.set(
				this,
				(__runInitializers(this, _excludeReferences_extraInitializers),
				__runInitializers(this, _commentStyle_initializers, void 0)),
			);
			_Converter_validation_accessor_storage.set(
				this,
				(__runInitializers(this, _commentStyle_extraInitializers),
				__runInitializers(this, _validation_initializers, void 0)),
			);
			_Converter_externalSymbolLinkMappings_accessor_storage.set(
				this,
				(__runInitializers(this, _validation_extraInitializers),
				__runInitializers(
					this,
					_externalSymbolLinkMappings_initializers,
					void 0,
				)),
			);
			_Converter_preserveLinkText_accessor_storage.set(
				this,
				(__runInitializers(
					this,
				...
				__runInitializers(
					this,
					_preserveLinkText_initializers,
					void 0,
				)),
			);
			_Converter_maxTypeConversionDepth_accessor_storage.set(
				this,
				(__runInitializers(this, _preserveLinkText_extraInitializers),
				__runInitializers(
					this,
					_maxTypeConversionDepth_initializers,
					void 0,
				)),
			);
			this._config = __runInitializers(
				this,
				_maxTypeConversionDepth_extraInitializers,
			);
			...
			this.addUnknownSymbolResolver((ref) => {
				...
				const modLinks =
					this.externalSymbolLinkMappings[
						ref.moduleSource ?? "global"
					];
				...
				if (ref.symbolReference.path) {
					name += ref.symbolReference.path
						.map((p) => p.path)
						.join(".");
				}
				if (ref.symbolReference.meaning) {
					name += (0, declarationReference_1.meaningToString)(
						ref.symbolReference.meaning,
					);
				}
		 ...
		 */
		convert(entryPoints) {
			const programs = (0, utils_1.unique)(
				entryPoints.map((e) => e.program),
			);
			this.externalPatternCache = void 0;
			const project = new index_1.ProjectReflection(
		...
		addProjectDocuments(project) {
			const projectDocuments = (0, utils_1.getDocumentEntryPoints)(
			...
			for (const { displayName, path } of projectDocuments) {
				let file;
				try {
					file = new utils_1.MinimalSourceFile(
						(0, utils_1.readFile)(path),
						path,
					);
				} catch (error) {
		...
		parseRawComment(file, files) {
			return (0, parser_1.parseCommentString)(
				(0, rawLexer_1.lexCommentString)(file.text),
				this.config,
				file,
				this.application.logger,
				files,
		...
		resolveLinks(comment, owner) {
			if (comment instanceof index_1.Comment) {
				...
				return (0, linkResolver_1.resolvePartLinks)(
					owner,
					comment,
					(ref, part, refl, id) =>
						this.resolveExternalLink(ref, part, refl, id),
		...
		compile(entryPoints, context) {
			...
			if (!createModuleReflections) {
				...
				createModuleReflections = opts.isSet(
			...
			if (createModuleReflections === false) {
				...
				if (entryPoint.readmeFile) {
					...
					const { content } = this.parseRawComment(
						new utils_1.MinimalSourceFile(
							readme,
							entryPoint.readmeFile,
						),
		...
		convertReExports(moduleContext, node) {
			for (const exp of getExports(
				moduleContext,
				node,
				moduleContext.project.getSymbolFromReflection(
					moduleContext.scope,
				),
			).filter(
				(exp) =>
					!isDirectExport(
						moduleContext.resolveAliasedSymbol(exp),
						node,
					),
			)) {
				this.convertSymbol(moduleContext, exp);
		...
		isExcluded(symbol) {
			this.excludeCache ??= (0, paths_1.createMinimatch)(
				this.application.options.getValue("exclude"),
			);
			const cache = this.excludeCache;
			return (symbol.getDeclarations() ?? []).some((node) =>
				(0, paths_1.matchesAny)(cache, node.getSourceFile().fileName),
			);
		}
		/** @internal */
		isExternal(symbol) {
			this.externalPatternCache ??= (0, paths_1.createMinimatch)(
		...
		processDocumentTags(reflection, parent) {
			...
			if (relativeTo) {
				...
				for (const tag of tags) {
					const path = index_1.Comment.combineDisplayParts(
						tag.content,
					);
					let file;
					try {
					...
					this.addDocument(
						parent,
						file,
						(0, path_1.basename)(file.fileName).replace(
							/\.[^.]+$/,
							"",
						),
					);
		...
		addDocument(parent, file, displayName) {
			...
			const docRefl = new index_1.DocumentReflection(
				displayName,
				parent,
				content,
				frontmatter,
						...
						if (typeof child === "string") {
							...
							this.application.logger.error(
								this.application.i18n.frontmatter_children_0_should_be_an_array_of_strings_or_object_with_string_values(
									(0, paths_1.nicePath)(file.fileName),
								),
							);
							return;
						}
			...
			for (const [displayName, path] of childrenToAdd) {
				const absPath = (0, path_1.resolve)(
					(0, path_1.dirname)(file.fileName),
					path,
				);
				let childFile;
				try {
					childFile = new utils_1.MinimalSourceFile(
						(0, utils_1.readFile)(absPath),
						absPath,
					);
				} catch (error) {
					this.application.logger.error(
		...
		_buildCommentParserConfig() {
			this._config = {
				blockTags: new Set(
					this.application.options.getValue("blockTags"),
				),
				inlineTags: new Set(
	...
	(() => {
		...
		_excludeExternals_decorators = [
			(0, utils_1.Option)("excludeExternals"),
		];
		_excludeNotDocumented_decorators = [
			(0, utils_1.Option)("excludeNotDocumented"),
		];
		_excludePrivate_decorators = [(0, utils_1.Option)("excludePrivate")];
		_excludeProtected_decorators = [
			(0, utils_1.Option)("excludeProtected"),
		];
		_excludeReferences_decorators = [
			(0, utils_1.Option)("excludeReferences"),
		];
		_commentStyle_decorators = [(0, utils_1.Option)("commentStyle")];
		_validation_decorators = [(0, utils_1.Option)("validation")];
		_externalSymbolLinkMappings_decorators = [
			(0, utils_1.Option)("externalSymbolLinkMappings"),
		];
		_preserveLinkText_decorators = [
			(0, utils_1.Option)("preserveLinkText"),
		];
		_maxTypeConversionDepth_decorators = [
			(0, utils_1.Option)("maxTypeConversionDepth"),
		];
		__esDecorate(
			_classThis,
			null,
			_externalPattern_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "externalPattern" in obj,
					get: (obj) => obj.externalPattern,
					set: (obj, value) => {
						obj.externalPattern = value;
					},
				},
				metadata: _metadata,
			},
			_externalPattern_initializers,
			_externalPattern_extraInitializers,
		);
		__esDecorate(
			_classThis,
			...
			_excludeExternals_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "excludeExternals" in obj,
					get: (obj) => obj.excludeExternals,
					set: (obj, value) => {
						obj.excludeExternals = value;
					},
				},
				metadata: _metadata,
			},
			_excludeExternals_initializers,
		...
		__esDecorate(
			...
			_excludeNotDocumented_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "excludeNotDocumented" in obj,
					get: (obj) => obj.excludeNotDocumented,
					set: (obj, value) => {
						obj.excludeNotDocumented = value;
					},
				},
				metadata: _metadata,
			},
		...
		__esDecorate(
			...
			_excludePrivate_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "excludePrivate" in obj,
					get: (obj) => obj.excludePrivate,
					set: (obj, value) => {
						obj.excludePrivate = value;
					},
				},
				metadata: _metadata,
			},
			_excludePrivate_initializers,
			_excludePrivate_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_excludeProtected_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "excludeProtected" in obj,
					get: (obj) => obj.excludeProtected,
					set: (obj, value) => {
						obj.excludeProtected = value;
					},
				},
				metadata: _metadata,
			},
			_excludeProtected_initializers,
			_excludeProtected_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_excludeReferences_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "excludeReferences" in obj,
					get: (obj) => obj.excludeReferences,
					set: (obj, value) => {
						obj.excludeReferences = value;
					},
				},
				metadata: _metadata,
			},
		...
		__esDecorate(
			...
			_commentStyle_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "commentStyle" in obj,
					get: (obj) => obj.commentStyle,
					set: (obj, value) => {
						obj.commentStyle = value;
					},
				},
				metadata: _metadata,
			},
			_commentStyle_initializers,
			_commentStyle_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_validation_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "validation" in obj,
					get: (obj) => obj.validation,
					set: (obj, value) => {
			...
			_validation_initializers,
			_validation_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_externalSymbolLinkMappings_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "externalSymbolLinkMappings" in obj,
					get: (obj) => obj.externalSymbolLinkMappings,
					set: (obj, value) => {
						obj.externalSymbolLinkMappings = value;
					},
				},
				metadata: _metadata,
			},
		...
		__esDecorate(
			...
			_preserveLinkText_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "preserveLinkText" in obj,
					get: (obj) => obj.preserveLinkText,
					set: (obj, value) => {
						obj.preserveLinkText = value;
					},
				},
				metadata: _metadata,
			},
			_preserveLinkText_initializers,
			_preserveLinkText_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_maxTypeConversionDepth_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "maxTypeConversionDepth" in obj,
					get: (obj) => obj.maxTypeConversionDepth,
					set: (obj, value) => {
						obj.maxTypeConversionDepth = value;
					},
				},
				metadata: _metadata,
			},
	 ...
	 * @event
	 */
	_classThis.EVENT_CREATE_DECLARATION =
		converter_events_1.ConverterEvents.CREATE_DECLARATION;
	/**
	 ...
	 * @event
	 */
	_classThis.EVENT_CREATE_SIGNATURE =
		converter_events_1.ConverterEvents.CREATE_SIGNATURE;
	/**
	 * Triggered when the converter has created a parameter reflection.
	 * The listener will be given {@link Context}, {@link Models.ParameterReflection} and a `ts.Node?`
	 * @event
	 */
	_classThis.EVENT_CREATE_PARAMETER =
		converter_events_1.ConverterEvents.CREATE_PARAMETER;
	/**
	 * Triggered when the converter has created a type parameter reflection.
	 * The listener will be given {@link Context} and a {@link Models.TypeParameterReflection}
	 * @event
	 */
	_classThis.EVENT_CREATE_TYPE_PARAMETER =
		converter_events_1.ConverterEvents.CREATE_TYPE_PARAMETER;
	/**
	 * Resolve events
	 */
	/**
	 ...
	 * @event
	 */
	_classThis.EVENT_RESOLVE_BEGIN =
		converter_events_1.ConverterEvents.RESOLVE_BEGIN;
	/**
	 ...
	 * @event
	 */
	_classThis.EVENT_RESOLVE_END =
		converter_events_1.ConverterEvents.RESOLVE_END;
	(() => {
...
function getSymbolForModuleLike(context, node) {
	...
	if (
		...
		globalSymbols[0]
			...
			?.every(
				(declaration) =>
					typescript_1.default.isModuleDeclaration(declaration) &&
...
function getExports(context, node, symbol) {
	...
	if (exportEq) {
					...
					(s) =>
						!(0, enum_1.hasAnyFlag)(
							...
							typescript_1.default.SymbolFlags.Prototype |
								typescript_1.default.SymbolFlags.Value,
						),
				),
		);
	} else if (symbol) {
		...
		result = context.checker
			.getSymbolsInScope(
				node,
				typescript_1.default.SymbolFlags.ModuleMember,
			)
			.filter((s) =>
				s
					.getDeclarations()

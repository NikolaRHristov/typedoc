 ...
 */
let CategoryPlugin = (() => {
	var _CategoryPlugin_defaultCategory_accessor_storage,
		_CategoryPlugin_categoryOrder_accessor_storage,
		_CategoryPlugin_categorizeByGroup_accessor_storage,
		_CategoryPlugin_boosts_accessor_storage;
	...
	var CategoryPlugin = (_classThis = class extends _classSuper {
		constructor() {
			super(...arguments);
			_CategoryPlugin_defaultCategory_accessor_storage.set(
				this,
				__runInitializers(this, _defaultCategory_initializers, void 0),
			);
			_CategoryPlugin_categoryOrder_accessor_storage.set(
				this,
				(__runInitializers(this, _defaultCategory_extraInitializers),
				__runInitializers(this, _categoryOrder_initializers, void 0)),
			);
			_CategoryPlugin_categorizeByGroup_accessor_storage.set(
				this,
				(__runInitializers(this, _categoryOrder_extraInitializers),
				__runInitializers(
					this,
					_categorizeByGroup_initializers,
					void 0,
				)),
			);
			_CategoryPlugin_boosts_accessor_storage.set(
				this,
				(__runInitializers(this, _categorizeByGroup_extraInitializers),
				__runInitializers(this, _boosts_initializers, void 0)),
			);
			this.usedBoosts =
				(__runInitializers(this, _boosts_extraInitializers), new Set());
		}
		get defaultCategory() {
			return __classPrivateFieldGet(
				this,
				_CategoryPlugin_defaultCategory_accessor_storage,
				"f",
			);
		}
		...
		initialize() {
			...
			this.owner.on(
				converter_1.Converter.EVENT_RESOLVE_END,
		 ...
		 */
		onBegin(_context) {
			this.sortFunction = (0, utils_1.getSortFunction)(
			...
			if (parent.comment) {
				(0, utils_1.removeIf)(parent.comment.blockTags, (tag) => {
					if (tag.tag === "@categoryDescription") {
						const { header, body } =
							models_1.Comment.splitPartsToHeaderAndBody(
								tag.content,
							);
		...
		extractCategories(reflection) {
			...
			if (reflection.isDeclaration()) {
				...
				if (reflection.type?.type === "reflection") {
					reflection.type.declaration.comment?.removeTags(
						"@category",
					);
			...
			if (
				...
				"category" in reflection.frontmatter
			) {
	...
	(() => {
		...
		_categorizeByGroup_decorators = [
			(0, utils_1.Option)("categorizeByGroup"),
		];
		...
		__esDecorate(
			...
			_categoryOrder_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "categoryOrder" in obj,
					get: (obj) => obj.categoryOrder,
					set: (obj, value) => {
						obj.categoryOrder = value;
					},
				},
				metadata: _metadata,
			},
			_categoryOrder_initializers,
			_categoryOrder_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_categorizeByGroup_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "categorizeByGroup" in obj,
					get: (obj) => obj.categorizeByGroup,
					set: (obj, value) => {
						obj.categorizeByGroup = value;
					},
				},
				metadata: _metadata,
			},
			_categorizeByGroup_initializers,
			_categorizeByGroup_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			...
			_boosts_initializers,
			_boosts_extraInitializers,
		);
		__esDecorate(
			null,

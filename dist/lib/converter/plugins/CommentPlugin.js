...
const MUTUALLY_EXCLUSIVE_MODIFIERS = [
	new Set(["@alpha", "@beta", "@experimental", "@internal", "@public"]),
];
/**
 ...
 */
let CommentPlugin = (() => {
	var _CommentPlugin_excludeTags_accessor_storage,
		_CommentPlugin_cascadedModifierTags_accessor_storage,
		_CommentPlugin_excludeInternal_accessor_storage,
		_CommentPlugin_excludePrivate_accessor_storage,
		_CommentPlugin_excludeProtected_accessor_storage,
		_CommentPlugin_excludeNotDocumented_accessor_storage,
	...
	var CommentPlugin = (_classThis = class extends _classSuper {
		constructor() {
			super(...arguments);
			_CommentPlugin_excludeTags_accessor_storage.set(
				this,
				__runInitializers(this, _excludeTags_initializers, void 0),
			);
			_CommentPlugin_cascadedModifierTags_accessor_storage.set(
				this,
				(__runInitializers(this, _excludeTags_extraInitializers),
				__runInitializers(
					this,
					_cascadedModifierTags_initializers,
					void 0,
				)),
			);
			_CommentPlugin_excludeInternal_accessor_storage.set(
				this,
				(__runInitializers(
					this,
					_cascadedModifierTags_extraInitializers,
				),
				__runInitializers(this, _excludeInternal_initializers, void 0)),
			);
			_CommentPlugin_excludePrivate_accessor_storage.set(
				this,
				(__runInitializers(this, _excludeInternal_extraInitializers),
				__runInitializers(this, _excludePrivate_initializers, void 0)),
			);
			_CommentPlugin_excludeProtected_accessor_storage.set(
				this,
			...
			_CommentPlugin_excludeNotDocumented_accessor_storage.set(
				this,
			...
			_CommentPlugin_excludeCategories_accessor_storage.set(
				this,
				(__runInitializers(
					this,
				...
				__runInitializers(
					this,
					_excludeCategories_initializers,
					void 0,
				)),
			);
			_CommentPlugin_defaultCategory_accessor_storage.set(
				this,
				(__runInitializers(this, _excludeCategories_extraInitializers),
				__runInitializers(this, _defaultCategory_initializers, void 0)),
			);
			this._excludeKinds = __runInitializers(
				this,
		...
		get excludeTags() {
			return __classPrivateFieldGet(
				this,
				_CommentPlugin_excludeTags_accessor_storage,
				"f",
			);
		}
		...
		set cascadedModifierTags(value) {
			__classPrivateFieldSet(
				this,
				_CommentPlugin_cascadedModifierTags_accessor_storage,
				value,
		...
		initialize() {
			...
			this.owner.on(
				converter_1.Converter.EVENT_RESOLVE,
			...
			if (comment.hasModifier("@protected")) {
				...
				if (reflection.kindOf(models_1.ReflectionKind.CallSignature)) {
					reflection.parent?.setFlag(
						models_1.ReflectionFlag.Protected,
					);
				}
			...
			if (
				reflection.kindOf(
					models_1.ReflectionKind.Project |
						models_1.ReflectionKind.SomeModule,
				)
			) {
		...
		onCreateTypeParameter(_context, reflection) {
			...
			if (comment) {
				let tag = comment.getIdentifiedTag(
					reflection.name,
					"@typeParam",
				);
				if (!tag) {
		...
		onDeclaration(_context, reflection) {
			...
			if (reflection.kindOf(models_1.ReflectionKind.SomeModule)) {
				...
				if (tag) {
					...
					const newName = models_1.Comment.combineDisplayParts(
						tag.content,
					).trim();
		...
		onBeginResolve(context) {
			...
			for (const ref of reflections) {
				if (
					ref.kindOf(models_1.ReflectionKind.Accessor) &&
					ref.flags.isReadonly
				) {
					const decl = ref;
			...
			const [allRemoved, someRemoved] = (0, utils_1.partition)(
				(0, utils_1.unique)(
					(0, utils_1.filterMap)(hidden, (reflection) =>
						reflection.parent?.kindOf(
							models_1.ReflectionKind.SignatureContainer,
						)
							? reflection.parent
							: void 0,
					),
		...
		onResolve(context, reflection) {
			if (reflection.comment) {
				...
				for (const group of MUTUALLY_EXCLUSIVE_MODIFIERS) {
					const intersect = (0, set_1.setIntersection)(
						group,
			...
			if (
				(reflection instanceof models_1.DeclarationReflection ||
			...
			signature.parameters?.forEach((parameter, index) => {
				...
				if (tag) {
					parameter.comment = new models_1.Comment(
						models_1.Comment.cloneDisplayParts(tag.content),
					);
		...
		cascadeModifiers(reflection) {
			...
			for (const mod of this.cascadedModifierTags) {
				if (parentComment.hasModifier(mod)) {
					const exclusiveSet = MUTUALLY_EXCLUSIVE_MODIFIERS.find(
						(tags) => tags.has(mod),
					);
					if (
						!exclusiveSet ||
						Array.from(exclusiveSet).every(
							(tag) => !childMods.has(tag),
						)
					) {
		...
		isHidden(reflection) {
			const comment = reflection.comment;
			if (
				reflection.flags.hasFlag(models_1.ReflectionFlag.Private) &&
				this.excludePrivate
			) {
				return true;
			}
			if (
				reflection.flags.hasFlag(models_1.ReflectionFlag.Protected) &&
			...
			if (
				!isHidden &&
				reflection.kindOf(
					models_1.ReflectionKind.ContainsCallSignatures,
				)
			) {
				return reflection
					.getNonIndexSignatures()
		...
		excludedByCategory(reflection) {
			...
			const categories =
				CategoryPlugin_1.CategoryPlugin.getCategories(target);
		...
		validateParamTags(signature, comment, params) {
			...
			(0, utils_1.removeIf)(paramTags, (tag) =>
	...
	(() => {
		...
		_cascadedModifierTags_decorators = [
			(0, utils_1.Option)("cascadedModifierTags"),
		];
		...
		_excludeCategories_decorators = [
			(0, utils_1.Option)("excludeCategories"),
		];
		_defaultCategory_decorators = [(0, utils_1.Option)("defaultCategory")];
		__esDecorate(
			_classThis,
			null,
			_excludeTags_decorators,
			{
				kind: "accessor",
				name: "excludeTags",
				static: false,
				private: false,
				access: {
					has: (obj) => "excludeTags" in obj,
					get: (obj) => obj.excludeTags,
					set: (obj, value) => {
						obj.excludeTags = value;
					},
				},
				metadata: _metadata,
			},
			_excludeTags_initializers,
			_excludeTags_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_cascadedModifierTags_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "cascadedModifierTags" in obj,
					get: (obj) => obj.cascadedModifierTags,
					set: (obj, value) => {
						obj.cascadedModifierTags = value;
					},
				},
				metadata: _metadata,
			},
		...
		__esDecorate(
			...
			_excludeInternal_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "excludeInternal" in obj,
					get: (obj) => obj.excludeInternal,
					set: (obj, value) => {
						obj.excludeInternal = value;
					},
				},
				metadata: _metadata,
			},
			_excludeInternal_initializers,
		...
		__esDecorate(
			...
			_excludeCategories_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "excludeCategories" in obj,
					get: (obj) => obj.excludeCategories,
					set: (obj, value) => {
						obj.excludeCategories = value;
					},
				},
				metadata: _metadata,
			},
			_excludeCategories_initializers,
			_excludeCategories_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_defaultCategory_decorators,
			{
				kind: "accessor",
				...
				private: false,
				access: {
					has: (obj) => "defaultCategory" in obj,
					get: (obj) => obj.defaultCategory,
					set: (obj, value) => {
						obj.defaultCategory = value;
					},
				},
				metadata: _metadata,
			},
			_defaultCategory_initializers,
			_defaultCategory_extraInitializers,
		);
		__esDecorate(
			null,
...
function moveNestedParamTags(/* in-out */ paramTags, parameters, sourcePath) {
	const used = new Set();
			...
			reflection(target) {
				...
				for (const tag of tags) {
					...
					if (child && !child.comment) {
						child.comment = new models_1.Comment(
							models_1.Comment.cloneDisplayParts(tag.content),
						);
...
function movePropertyTags(comment, container) {
	...
	for (const prop of propTags) {
		...
		if (child) {
			child.comment = new models_1.Comment(
				models_1.Comment.cloneDisplayParts(prop.content),
			);
			child.comment.sourcePath = comment.sourcePath;
			if (
				child instanceof models_1.DeclarationReflection &&
				child.signatures
			) {
				for (const sig of child.signatures) {
					sig.comment = new models_1.Comment(
						models_1.Comment.cloneDisplayParts(prop.content),
					);

	...
	var LinkResolverPlugin = (_classThis = class extends _classSuper {
		get validation() {
			return __classPrivateFieldGet(
				this,
				_LinkResolverPlugin_validation_accessor_storage,
				"f",
			);
		}
		...
		resolveLinks(project) {
			for (const id in project.reflections) {
				...
				if (reflection.isDeclaration()) {
					reflection.type?.visit(
						(0, models_1.makeRecursiveVisitor)({
							union: (type) => {
								type.elementSummaries =
									type.elementSummaries?.map((parts) =>
										this.owner.resolveLinks(
											parts,
											reflection,
										),
									);
			...
			if (project.readme) {
				project.readme = this.owner.resolveLinks(
					project.readme,
					project,
				);
			}
			for (const { type, owner } of (0,
			reflections_1.discoverAllReferenceTypes)(project, false)) {
				if (!type.reflection) {
		...
		constructor() {
			super(...arguments);
			_LinkResolverPlugin_validation_accessor_storage.set(
				this,

...
let ImplementsPlugin = (() => {
	...
	var ImplementsPlugin = (_classThis = class extends _classSuper {
		...
		analyzeImplements(project, classReflection, interfaceReflection) {
			...
			interfaceReflection.children.forEach((interfaceMember) => {
				...
				classMember.implementationOf =
					types_1.ReferenceType.createResolvedReference(
						interfaceMemberName,
						interfaceMember,
						project,
					);
				...
				if (intSigs && clsSigs) {
					for (const [clsSig, intSig] of (0, array_1.zip)(
						clsSigs,
						intSigs,
					)) {
						if (clsSig.implementationOf) {
							const target = intSig.parent.kindOf(
								index_1.ReflectionKind.FunctionOrMethod,
							)
								? intSig
		...
		analyzeInheritance(project, reflection) {
			const extendedTypes = (0, array_1.filterMap)(
				...
				for (const parentMember of parent.reflection.children ?? []) {
					...
					if (child) {
						const key = child.overwrites
							? "overwrites"
							: "inheritedFrom";
						for (const [childSig, parentSig] of (0, array_1.zip)(
							child.signatures ?? [],
		...
		doResolve(project, reflection) {
			if (
				reflection.kindOf(index_1.ReflectionKind.Class) &&
				reflection.implementedTypes
			) {
				reflection.implementedTypes.forEach((type) => {
					...
					if (
						type.reflection &&
						type.reflection.kindOf(
							index_1.ReflectionKind.ClassOrInterface,
						)
					) {
			...
			if (
				reflection.kindOf(index_1.ReflectionKind.ClassOrInterface) &&
				reflection.extendedTypes
			) {
		...
		getExtensionInfo(context, reflection) {
			if (
				!reflection ||
				!reflection.kindOf(index_1.ReflectionKind.Inheritable)
			) {
				return;
			}
			...
			const declaration = symbol
				...
				?.find(
					(n) =>
						typescript_1.default.isClassDeclaration(n) ||
						typescript_1.default.isInterfaceDeclaration(n),
				);
			if (!declaration) {
				return;
			}
		...
		handleInheritedComments(child, parent) {
			...
			if (
				parent.kindOf(index_1.ReflectionKind.Property) &&
				child.kindOf(index_1.ReflectionKind.Accessor)
			) {
				if (child.getSignature) {
					this.copyComment(child.getSignature, parent);
					child.getSignature.implementationOf =
						child.implementationOf;
				}
				if (child.setSignature) {
					this.copyComment(child.setSignature, parent);
					child.setSignature.implementationOf =
						child.implementationOf;
				}
			}
			if (
				parent.kindOf(index_1.ReflectionKind.Accessor) &&
				child.kindOf(index_1.ReflectionKind.Accessor)
			) {
			...
			if (
				parent.kindOf(index_1.ReflectionKind.FunctionOrMethod) &&
				parent.signatures &&
				child.signatures
			) {
				for (const [cs, ps] of (0, array_1.zip)(
					child.signatures,
					parent.signatures,
				)) {
		...
		copyComment(target, source) {
			...
			if (
				target instanceof index_1.DeclarationReflection &&
				source instanceof index_1.DeclarationReflection
			) {
				for (const [tt, ts] of (0, array_1.zip)(
			...
			if (
				target instanceof index_1.SignatureReflection &&
				...
				for (const [pt, ps] of (0, array_1.zip)(
					target.parameters || [],
					source.parameters || [],
				)) {
...
function constructorInheritance(
	...
	const extendsClause = childDecl.heritageClauses?.find(
		(cl) => cl.token === typescript_1.default.SyntaxKind.ExtendsKeyword,
	);
	if (!extendsClause) return;
	const name = `${extendsClause.types[0].getText()}.constructor`;
	const key = constructorDecl ? "overwrites" : "inheritedFrom";
	reflection[key] ??= types_1.ReferenceType.createBrokenReference(
		name,
		context.project,
	);
	for (const sig of reflection.signatures ?? []) {
		sig[key] ??= types_1.ReferenceType.createBrokenReference(
			name,
			context.project,
		);
	}
	...
	function link(target) {
		...
		if (
			...
			target.implementationOf ??=
				types_1.ReferenceType.createBrokenReference(name, project);
			return;
		}
		if (isInherit) {
			...
			target.overwrites ??= types_1.ReferenceType.createBrokenReference(
				name,
				project,
			);
		}

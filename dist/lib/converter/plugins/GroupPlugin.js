 ...
 */
let GroupPlugin = (() => {
	var _GroupPlugin_boosts_accessor_storage,
		_GroupPlugin_groupOrder_accessor_storage,
		_GroupPlugin_sortEntryPoints_accessor_storage;
	...
	var GroupPlugin = (_classThis = class extends _classSuper {
		constructor() {
			super(...arguments);
			_GroupPlugin_boosts_accessor_storage.set(
				this,
				__runInitializers(this, _boosts_initializers, void 0),
			);
			_GroupPlugin_groupOrder_accessor_storage.set(
				this,
				(__runInitializers(this, _boosts_extraInitializers),
				__runInitializers(this, _groupOrder_initializers, void 0)),
			);
			_GroupPlugin_sortEntryPoints_accessor_storage.set(
				this,
				(__runInitializers(this, _groupOrder_extraInitializers),
				__runInitializers(this, _sortEntryPoints_initializers, void 0)),
			);
			this.usedBoosts =
				(__runInitializers(this, _sortEntryPoints_extraInitializers),
				new Set());
		}
		get boosts() {
			return __classPrivateFieldGet(
				this,
				_GroupPlugin_boosts_accessor_storage,
				"f",
			);
		}
		set boosts(value) {
		...
		initialize() {
			this.owner.on(
				...
				() => {
					this.sortFunction = (0, sort_1.getSortFunction)(
				...
				if (reflection.children) {
					if (
						...
						!reflection.children.some((c) =>
							c.kindOf(index_1.ReflectionKind.Module),
						)
					) {
		...
		getReflectionGroups(parent, reflections) {
			...
			reflections.forEach((child) => {
				for (const name of this.getGroups(child)) {
					...
					if (!group) {
						group = new ReflectionGroup_1.ReflectionGroup(
							name,
							child,
						);
	...
	(() => {
		...
		__esDecorate(
			...
			_groupOrder_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "groupOrder" in obj,
					get: (obj) => obj.groupOrder,
					set: (obj, value) => {
						obj.groupOrder = value;
					},
				},
				metadata: _metadata,
			},
			_groupOrder_initializers,
			_groupOrder_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_sortEntryPoints_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "sortEntryPoints" in obj,
					get: (obj) => obj.sortEntryPoints,
					set: (obj, value) => {
						obj.sortEntryPoints = value;
					},
				},
				metadata: _metadata,
			},
			_sortEntryPoints_initializers,
			_sortEntryPoints_extraInitializers,
		);
		__esDecorate(
			null,

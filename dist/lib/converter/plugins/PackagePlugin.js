 ...
 */
let PackagePlugin = (() => {
	var _PackagePlugin_readme_accessor_storage,
		_PackagePlugin_entryPointStrategy_accessor_storage,
		_PackagePlugin_entryPoints_accessor_storage,
		_PackagePlugin_includeVersion_accessor_storage;
	...
	var PackagePlugin = (_classThis = class extends _classSuper {
		constructor() {
			super(...arguments);
			_PackagePlugin_readme_accessor_storage.set(
				this,
				__runInitializers(this, _readme_initializers, void 0),
			);
			_PackagePlugin_entryPointStrategy_accessor_storage.set(
				this,
				(__runInitializers(this, _readme_extraInitializers),
				__runInitializers(
					this,
			...
			_PackagePlugin_entryPoints_accessor_storage.set(
				this,
				(__runInitializers(this, _entryPointStrategy_extraInitializers),
				__runInitializers(this, _entryPoints_initializers, void 0)),
			);
			_PackagePlugin_includeVersion_accessor_storage.set(
				this,
				(__runInitializers(this, _entryPoints_extraInitializers),
				__runInitializers(this, _includeVersion_initializers, void 0)),
			);
			/**
			 * The file name of the found readme.md file.
			 */
			this.readmeFile = __runInitializers(
				this,
				_includeVersion_extraInitializers,
			);
		}
		get readme() {
			return __classPrivateFieldGet(
				this,
				_PackagePlugin_readme_accessor_storage,
				"f",
			);
		}
		set readme(value) {
		...
		initialize() {
			this.owner.on(
				converter_1.Converter.EVENT_BEGIN,
		...
		onBegin() {
			...
			const entryFiles =
				this.entryPointStrategy === utils_1.EntryPointStrategy.Packages
					? this.entryPoints.map((d) =>
							(0, path_1.join)(d, "package.json"),
						)
					: this.entryPoints;
			const dirName =
			...
			this.application.logger.verbose(
				`Begin readme.md/package.json search at ${(0, paths_1.nicePath)(dirName)}`,
			);
				...
				try {
					...
					this.application.logger.error(
						this.application.i18n.provided_readme_at_0_could_not_be_read(
							(0, paths_1.nicePath)(this.readme),
						),
					);
				}
			} else {
				// No readme provided, automatically find the readme
				const result = (0, fs_1.discoverInParentDir)(
					"readme.md",
					dirName,
					(content) => content,
				);
		...
		addEntries(project) {
			if (this.readmeFile && this.readmeContents) {
				const { content } = this.application.converter.parseRawComment(
					new minimalSourceFile_1.MinimalSourceFile(
						this.readmeContents,
						this.readmeFile,
					),
					project.files,
	...
	(() => {
		...
		__esDecorate(
			...
			_readme_initializers,
			_readme_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
		...
		__esDecorate(
			...
			_includeVersion_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "includeVersion" in obj,
					get: (obj) => obj.includeVersion,
					set: (obj, value) => {
						obj.includeVersion = value;
					},
				},
				metadata: _metadata,
			},
			_includeVersion_initializers,
			_includeVersion_extraInitializers,
		);
		__esDecorate(
			null,

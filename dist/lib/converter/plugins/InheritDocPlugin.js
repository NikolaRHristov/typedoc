...
let InheritDocPlugin = (() => {
	...
	var InheritDocPlugin = (_classThis = class extends _classSuper {
		constructor() {
			super(...arguments);
			_InheritDocPlugin_validation_accessor_storage.set(
				this,
			...
			this.dependencies =
				...
				new utils_1.DefaultMap(() => []));
		}
		get validation() {
			return __classPrivateFieldGet(
				this,
				_InheritDocPlugin_validation_accessor_storage,
				"f",
			);
		}
		 ...
		 */
		initialize() {
			this.owner.on(converter_1.Converter.EVENT_RESOLVE_END, (context) =>
		...
		processInheritDoc(project) {
			for (const id in project.reflections) {
				...
				if (!source) continue;
				const declRef = (0,
				declarationReference_1.parseDeclarationReference)(
					source,
					0,
					source.length,
				);
				...
				let sourceRefl =
					declRef &&
					(0,
					declarationReferenceResolver_1.resolveDeclarationReference)(
						reflection,
						declRef[0],
					);
				...
				if (
					sourceRefl instanceof models_1.DeclarationReflection &&
					...
					sourceRefl =
						sourceRefl.getSignature || sourceRefl.setSignature;
				}
				if (!sourceRefl) {
		...
		copyComment(source, target) {
			...
			target.comment.summary = models_1.Comment.cloneDisplayParts(

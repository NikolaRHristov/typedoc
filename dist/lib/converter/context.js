...
class Context {
	...
	expectSymbolAtLocation(node) {
		...
		if (!symbol) {
			...
			throw new Error(
				`Expected a symbol for node with kind ${typescript_1.default.SyntaxKind[node.kind]} at ${node.getSourceFile().fileName}:${line + 1}`,
			);
		}
		return symbol;
	}
	...
	createDeclarationReflection(
		...
		const name = (0, tsutils_1.getHumanName)(
		...
		const reflection = new index_1.DeclarationReflection(
			name,
			kind,
			this.scope,
		);
	...
	postReflectionCreation(reflection, symbol, exportSymbol) {
		if (
			exportSymbol &&
			reflection.kind &
				(index_1.ReflectionKind.SomeModule |
		...
		const path = reflection.kindOf(
			index_1.ReflectionKind.Namespace | index_1.ReflectionKind.Module,
		)
			? symbol?.declarations?.find(typescript_1.default.isSourceFile)
					?.fileName
			: undefined;
	...
	getComment(symbol, kind) {
		return (0, comments_1.getComment)(
			symbol,
			kind,
	...
	getNodeComment(node, moduleComment) {
		return (0, comments_1.getNodeComment)(
			node,
			moduleComment,
			this.converter.config,
			this.logger,
	...
	getFileComment(node) {
		return (0, comments_1.getFileComment)(
			node,
			this.converter.config,
			this.logger,
			this.checker,
	...
	getJsDocComment(declaration) {
		return (0, comments_1.getJsDocComment)(
			declaration,
			this.converter.config,
			this.logger,
			this.checker,
	...
	getSignatureComment(declaration) {
		return (0, comments_1.getSignatureComment)(
			declaration,
			this.converter.config,
			this.logger,
			this.checker,

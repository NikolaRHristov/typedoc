	...
	return (
		(_a = class GitRepository {
			...
			constructor(path, gitRevision, urlTemplate) {
				 ...
				 */
				this.files = __runInitializers(
					this,
					_files_initializers,
					new Set(),
				);
				this.urlTemplate = __runInitializers(
					this,
					_files_extraInitializers,
				);
				this.path = path;
				this.gitRevision = gitRevision;
				...
				if (out.status === 0) {
					out.stdout.split("\0").forEach((file) => {
						if (file !== "") {
							this.files.add(
								(0, utils_1.normalizePath)(path + "/" + file),
							);
						}
					});
				}
			...
			static tryCreateRepository(
				...
				if (sourceLinkTemplate) {
					...
					if (remotesOut.status === 0) {
						...
						logger.warn(
							logger.i18n.git_remote_0_not_valid(gitRemote),
						);
					}
				}
				if (!urlTemplate) return;
				return new _a(
					(0, utils_1.normalizePath)(path),
					gitRevision,
					urlTemplate,
				);
			}
...
class RepositoryManager {
	...
	getRepository(fileName) {
		...
		return this.getRepositoryFolder(
			(0, utils_1.normalizePath)((0, path_1.dirname)(fileName)),
		);
	}
	getRepositoryFolder(dir) {
		...
		if ((0, fs_1.existsSync)((0, path_1.join)(dir, ".git"))) {
			...
			this.cache.set(
				dir,
				this.getRepositoryFolder((0, path_1.dirname)(dir)),
			);
		}
		return this.cache.get(dir);
	}

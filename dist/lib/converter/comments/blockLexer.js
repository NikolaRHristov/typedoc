		...
		switch (file[pos]) {
			...
			case "{":
				if (braceStartsType && nextNonWs(pos + 1) !== "@") {
					yield makeToken(
						lexer_1.TokenSyntaxKind.TypeAnnotation,
	...
	function attachLinkTagResult(token) {
		...
		if (
			...
			if (link.name) {
				...
				if (tsTarget) {
					token.tsLinkTarget =
						new ReflectionSymbolId_1.ReflectionSymbolId(
							(0, symbols_1.resolveAliasedSymbol)(
								tsTarget,
								checker,
							),
						);

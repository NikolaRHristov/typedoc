...
const wantedKinds = {
	...
	[models_1.ReflectionKind.IndexSignature]: [
		typescript_1.default.SyntaxKind.IndexSignature,
	],
	[models_1.ReflectionKind.ConstructorSignature]: [
		typescript_1.default.SyntaxKind.ConstructSignature,
	],
...
function discoverFileComments(node, commentStyle) {
	...
	const comments = collectCommentRanges(
		typescript_1.default.getLeadingCommentRanges(text, node.pos),
	);
...
function discoverComment(symbol, kind, logger, commentStyle, checker) {
	...
	const wantedDeclarations = (0, array_1.filter)(
		symbol.declarations,
		(decl) => wantedKinds[kind].includes(decl.kind),
	);
...
function findJsDocForComment(node, ranges) {
	if (
		...
		const jsDocs = typescript_1.default
			...
			.map((doc) =>
				typescript_1.default.findAncestor(
					doc,
					typescript_1.default.isJSDoc,
				),
			);
...
function declarationToCommentNodeIgnoringParents(node) {
	...
	if (
		node.parent.kind ===
		typescript_1.default.SyntaxKind.VariableDeclarationList
	) {
	...
	//           ^^^^^^^^
	if (
		node.parent.kind === typescript_1.default.SyntaxKind.VariableDeclaration
	) {
...
function findBaseOfDeclaration(checker, declaration, cb) {
	const classOrInterfaceDeclaration =
		declaration.parent?.kind === typescript_1.default.SyntaxKind.Constructor
			? declaration.parent.parent
			: declaration.parent;
	if (!classOrInterfaceDeclaration) return;
	const isStaticMember =
		typescript_1.default.getCombinedModifierFlags(declaration) &
		typescript_1.default.ModifierFlags.Static;
	return (0, array_1.firstDefined)(
		typescript_1.default.getAllSuperTypeNodes(classOrInterfaceDeclaration),
		(superTypeNode) => {

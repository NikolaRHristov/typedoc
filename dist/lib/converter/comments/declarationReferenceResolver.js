	...
	if (ref.moduleSource) {
		high =
			reflection.project.children?.filter(
				(c) =>
					c.kindOf(models_1.ReflectionKind.SomeModule) &&
		...
		if (reflection.kindOf(models_1.ReflectionKind.SomeMember)) {
			...
			reflection.kindOf(models_1.ReflectionKind.SomeSignature) &&
	...
	switch (kw) {
		case undefined:
			return refl instanceof models_1.DeclarationReflection &&
				...
				: [refl];
		case "class":
			if (refl.kindOf(models_1.ReflectionKind.Class)) return [refl];
			break;
		case "interface":
			if (refl.kindOf(models_1.ReflectionKind.Interface)) return [refl];
			break;
		case "type":
			if (refl.kindOf(models_1.ReflectionKind.SomeType)) return [refl];
			break;
		case "enum":
			if (refl.kindOf(models_1.ReflectionKind.Enum)) return [refl];
			break;
		case "namespace":
			if (refl.kindOf(models_1.ReflectionKind.SomeModule)) return [refl];
			break;
		case "function":
			...
			break;
		case "var":
			if (refl.kindOf(models_1.ReflectionKind.Variable)) return [refl];
			break;
		case "new":
		case "constructor":
			if (
				refl.kindOf(
					models_1.ReflectionKind.ClassOrInterface |
						models_1.ReflectionKind.TypeLiteral,
				)
			) {
				const ctor = refl.children?.find((c) =>
					c.kindOf(models_1.ReflectionKind.Constructor),
				);
				return ctor.signatures;
			}
			break;
		case "member":
			if (refl.kindOf(models_1.ReflectionKind.SomeMember)) return [refl];
			break;
		case "event":
...
function resolveSymbolReferencePart(refl, path) {
	...
	let low = [];
	if (
		!(refl instanceof models_1.ContainerReflection) ||
	...
	switch (path.navigation) {
		...
		case ".":
			high = refl.childrenIncludingDocuments.filter(
				(r) =>
					r.name === path.path &&
					(r.kindOf(models_1.ReflectionKind.SomeExport) ||
						r.flags.isStatic),
			);

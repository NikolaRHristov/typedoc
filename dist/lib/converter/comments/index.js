...
function getCommentWithCache(discovered, config, logger, checker, files) {
	...
	switch (ranges[0].kind) {
		case typescript_1.default.SyntaxKind.MultiLineCommentTrivia:
			comment = (0, parser_1.parseComment)(
				(0, blockLexer_1.lexBlockComment)(
					file.text,
					ranges[0].pos,
					ranges[0].end,
					jsDoc,
		...
		case typescript_1.default.SyntaxKind.SingleLineCommentTrivia:
			comment = (0, parser_1.parseComment)(
				(0, lineLexer_1.lexLineComments)(file.text, ranges),
				config,
				file,
				logger,
				files,
...
function getComment(symbol, kind, config, logger, checker, files) {
	...
	const isModule = declarations.some((decl) => {
		if (
			typescript_1.default.isModuleDeclaration(decl) &&
			typescript_1.default.isStringLiteral(decl.name)
		) {
			return true;
		}
		return false;
...
function getNodeComment(node, moduleComment, config, logger, checker, files) {
	return getCommentImpl(
		(0, discovery_1.discoverNodeComment)(node, config.commentStyle),
		config,
		logger,
		moduleComment,
		checker,
...
function getFileComment(file, config, logger, checker, files) {
	for (const commentSource of (0, discovery_1.discoverFileComments)(
		file,
		config.commentStyle,
	)) {
		const comment = getCommentWithCache(
...
function getSignatureComment(declaration, config, logger, checker, files) {
	return getCommentImpl(
		(0, discovery_1.discoverSignatureComment)(
			declaration,
			checker,
			config.commentStyle,
		),
...
function getJsDocComment(declaration, config, logger, checker, files) {
	...
	if (
		typescript_1.default.isJSDocTemplateTag(declaration) &&
		declaration.comment &&
	...
	if (!tag) {
		...
		const result = new models_1.Comment(

...
function resolveLinkTag(reflection, part, externalResolver, options) {
	...
	if (
		...
		part.target instanceof models_1.Reflection
	) {
		return part;
	}
	...
	while (
		pos < end &&
		typescript_1.default.isWhiteSpaceLike(part.text.charCodeAt(pos))
	) {
		pos++;
	}
	let target;
	// Try to parse a declaration reference if we didn't use the TS symbol for resolution
	const declRef = (0, declarationReference_1.parseDeclarationReference)(
		part.text,
		pos,
		end,
	);
	...
	if (part.target instanceof models_1.ReflectionSymbolId) {
		...
		if (tsTarget) {
			...
			const externalResolveResult = externalResolver(
				...
				reflection,
				part,
				part.target instanceof models_1.ReflectionSymbolId
					? part.target
					: undefined,
			);

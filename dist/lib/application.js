 ...
 */
let Application = (() => {
	var _Application_lang_accessor_storage,
		_Application_skipErrorChecking_accessor_storage,
		_Application_entryPointStrategy_accessor_storage,
		_Application_entryPoints_accessor_storage;
	...
	var Application = (_classThis = class extends _classSuper {
		...
		get lang() {
			return __classPrivateFieldGet(
				this,
				_Application_lang_accessor_storage,
				"f",
			);
		}
		set lang(value) {
		...
		constructor(detector) {
			...
			this.internationalization =
				new internationalization_1.Internationalization(this);
			/**
			...
			_Application_lang_accessor_storage.set(
				this,
				__runInitializers(this, _lang_initializers, void 0),
			);
			_Application_skipErrorChecking_accessor_storage.set(
				this,
				(__runInitializers(this, _lang_extraInitializers),
				__runInitializers(
					this,
					_skipErrorChecking_initializers,
					void 0,
				)),
			);
			_Application_entryPointStrategy_accessor_storage.set(
				this,
				(__runInitializers(this, _skipErrorChecking_extraInitializers),
				__runInitializers(
					this,
					_entryPointStrategy_initializers,
					void 0,
				)),
			);
			_Application_entryPoints_accessor_storage.set(
				this,
				(__runInitializers(this, _entryPointStrategy_extraInitializers),
				__runInitializers(this, _entryPoints_initializers, void 0)),
			);
		...
		async _bootstrap(options) {
			...
			this.trigger(
				application_events_1.ApplicationEvents.BOOTSTRAP_END,
				this,
			);
		...
		async convert() {
			...
			if (
				...
				entry_point_1.EntryPointStrategy.Merge
			) {
				return this._merge();
			}
			if (
			...
			if (
				!supportedVersionMajorMinor.some(
					(version) =>
						version == typescript_1.default.versionMajorMinor,
				)
			) {
				this.logger.warn(
			...
			const programs = (0, array_1.unique)(
		...
		convertAndWatch(success) {
			...
			if (this.options.getFileNames().length === 0) {
				this.logger.error(
					this.i18n.solution_not_supported_in_watch_mode(),
				);
				return;
			}
			...
			if (
				this.entryPointStrategy !==
					entry_point_1.EntryPointStrategy.Resolve &&
				this.entryPointStrategy !==
					entry_point_1.EntryPointStrategy.Expand
			) {
				this.logger.error(
					this.i18n.strategy_not_supported_in_watch_mode(),
				);
				return;
			}
			const tsconfigFile =
				(0, tsconfig_1.findTsConfigFile)(
			...
			const host = typescript_1.default.createWatchCompilerHost(
				...
				(status, newLine, _options, errorCount) => {
					...
					this.logger.info(
						typescript_1.default.flattenDiagnosticMessageText(
							status.messageText,
							newLine,
						),
					);
			...
			host.afterProgramCreate = (program) => {
				if (
					typescript_1.default.getPreEmitDiagnostics(
						program.getProgram(),
					).length === 0
				) {
		...
		validate(project) {
			...
			if (checks.notDocumented) {
				(0, documentation_1.validateDocumentation)(
					project,
					this.logger,
		...
		async generateDocs(project, out) {
			...
			if (this.logger.hasErrors()) {
				...
				this.logger.info(
					this.i18n.docs_generated_at_0((0, paths_1.nicePath)(out)),
				);
				this.logger.verbose(
					`HTML rendering took ${Date.now() - start}ms`,
				);
			}
		}
		/**
		...
		async generateJson(project, out) {
			...
			await (0, index_2.writeFile)(
				out,
				JSON.stringify(ser, null, space) + "\n",
			);
			this.logger.info(
				this.i18n.json_written_to_0((0, paths_1.nicePath)(out)),
			);
		...
		async _convertPackages() {
			...
			const packageDirs = (0, entry_point_1.getPackageDirectories)(
				this.logger,
				this.options,
			...
			for (const dir of packageDirs) {
				this.logger.verbose(
					`Reading project at ${(0, paths_1.nicePath)(dir)}`,
				);
				let opts;
				try {
				...
				if (
					...
					this.logger.error(
						this.i18n.nested_packages_unsupported_0(
							(0, paths_1.nicePath)(dir),
						),
					);
					continue;
				}
				(0, ReflectionSymbolId_1.addInferredDeclarationMapPaths)(
			...
			for (const { dir, options } of projectsToConvert) {
				this.logger.info(
					this.i18n.converting_project_at_0(
						(0, paths_1.nicePath)(dir),
					),
				);
		...
		_merge() {
			...
			const entryPoints = this.entryPoints.flatMap((entry) => {
				...
				if (result.length === 0) {
					this.logger.warn(
						this.i18n.entrypoint_did_not_match_files_0(
							(0, paths_1.nicePath)(entry),
						),
					);
				} else {
					this.logger.verbose(
						`Expanded ${(0, paths_1.nicePath)(entry)} to:\n\t${result
			...
			const jsonProjects = entryPoints.map((path) => {
				try {
					...
					this.logger.error(
						this.i18n.failed_to_parse_json_0(
			...
			this.logger.verbose(
				`Reviving projects took ${Date.now() - start}ms`,
			);
	...
	(() => {
		...
		_skipErrorChecking_decorators = [
			(0, utils_1.Option)("skipErrorChecking"),
		];
		_entryPointStrategy_decorators = [
			(0, utils_1.Option)("entryPointStrategy"),
		];
		...
		__esDecorate(
			...
			_skipErrorChecking_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "skipErrorChecking" in obj,
					get: (obj) => obj.skipErrorChecking,
					set: (obj, value) => {
						obj.skipErrorChecking = value;
					},
				},
				metadata: _metadata,
			},
			_skipErrorChecking_initializers,
			_skipErrorChecking_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_entryPointStrategy_decorators,
			{
				...
				private: false,
				access: {
					has: (obj) => "entryPointStrategy" in obj,
					get: (obj) => obj.entryPointStrategy,
					set: (obj, value) => {
						obj.entryPointStrategy = value;
					},
				},
				metadata: _metadata,
			},
			_entryPointStrategy_initializers,
			_entryPointStrategy_extraInitializers,
		);
		__esDecorate(
			_classThis,
			null,
			_entryPoints_decorators,
			{
				kind: "accessor",
				name: "entryPoints",
				static: false,
				private: false,
				access: {
					has: (obj) => "entryPoints" in obj,
					get: (obj) => obj.entryPoints,
					set: (obj, value) => {
						obj.entryPoints = value;
					},
				},
				metadata: _metadata,
			},
			_entryPoints_initializers,
			_entryPoints_extraInitializers,
		);
		__esDecorate(
			null,
	 ...
	 */
	_classThis.EVENT_BOOTSTRAP_END =
		application_events_1.ApplicationEvents.BOOTSTRAP_END;
	/**
	 * Emitted after a project has been deserialized from JSON.
	 * The listener will be given an instance of {@link ProjectReflection}.
	 */
	_classThis.EVENT_PROJECT_REVIVE =
		application_events_1.ApplicationEvents.REVIVE;
	/**
	 * Emitted when validation is being run.
	 * The listener will be given an instance of {@link ProjectReflection}.
	 */
	_classThis.EVENT_VALIDATE_PROJECT =
		application_events_1.ApplicationEvents.VALIDATE_PROJECT;
	(() => {

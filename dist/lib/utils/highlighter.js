...
exports.getSupportedLanguagesWithoutAliases =
...
async function loadShikiMetadata() {
	...
	supportedLanguagesWithoutAliases = (0, array_1.unique)([
		"text",
		...shiki.bundledLanguagesInfo.map((lang) => lang.id),
	]);
...
class DoubleHighlighter {
		...
		for (const line of tokens) {
			for (const token of line) {
				docEls.push(
					JSX.createElement(
						"span",
						{ class: this.getClass(token.variants) },
						token.content,
					),
				);
			}
	...
	getStyles() {
		...
		style.push(
			`    --light-code-background: ${this.highlighter.getTheme(this.light).bg};`,
		);
		style.push(
			`    --dark-code-background: ${this.highlighter.getTheme(this.dark).bg};`,
		);
...
function getSupportedLanguages() {
	(0, assert_1.ok)(
		...
		"loadShikiMetadata has not been called",
	);
	return supportedLanguages;
}
function getSupportedLanguagesWithoutAliases() {
	(0, assert_1.ok)(
		supportedLanguagesWithoutAliases.length > 0,
		"loadShikiMetadata has not been called",
	);
	return supportedLanguages;
}
...
function highlight(code, lang) {
	(0, assert_1.ok)(
		highlighter,
		"Tried to highlight with an uninitialized highlighter",
	);
	if (lang === "text") {

...
class Options {
	...
	reset(name) {
		if (name != null) {
			...
			this._values[declaration.name] = (0, declaration_2.getDefaultValue)(
				declaration,
			);
			this._setOptions.delete(declaration.name);
		} else {
			for (const declaration of this.getDeclarations()) {
				this._values[declaration.name] = (0,
				declaration_2.getDefaultValue)(declaration);
			}
	...
	setValue(name, value, configPath) {
		...
		const converted = (0, declaration_2.convert)(
			value,
			declaration,
			this._i18n,

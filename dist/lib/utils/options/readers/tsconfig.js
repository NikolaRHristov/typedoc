...
class TSConfigReader {
	...
	read(container, logger, cwd) {
		...
		if (!fileToRead) {
			...
			if (container.isSet("tsconfig")) {
				logger.error(
					logger.i18n.tsconfig_file_0_does_not_exist(
						(0, paths_1.nicePath)(file),
					),
				);
			}
			return;
		}
		fileToRead = (0, paths_1.normalizePath)(
			(0, path_1.resolve)(fileToRead),
		);
		logger.verbose(
			`Reading tsconfig at ${(0, paths_1.nicePath)(fileToRead)}`,
		);
		this.addTagsFromTsdocJson(
			container,
			logger,
			(0, path_1.resolve)(fileToRead),
		);
		...
		const typedocOptions = (0, tsconfig_1.getTypeDocOptionsFromTsConfig)(
			fileToRead,
		);
	...
	addTagsFromTsdocJson(container, logger, tsconfig) {
		...
		const tsdoc = (0, path_1.join)(
			(0, path_1.dirname)(tsconfig),
			"tsdoc.json",
		);
		if (!(0, fs_1.isFile)(tsdoc)) {
			return;
		}
		const overwritten = ["blockTags", "inlineTags", "modifierTags"].filter(
			(opt) => container.isSet(opt),
		);
	...
	readTsDoc(logger, path) {
		if (this.seenTsdocPaths.has(path)) {
			logger.error(
				logger.i18n.circular_reference_extends_0(
		...
		const { config, error } = typescript_1.default.readConfigFile(
			(0, paths_1.normalizePath)(path),
			typescript_1.default.sys.readFile,
		);
		if (error) {
			logger.error(
				logger.i18n.failed_read_tsdoc_json_0(
					(0, paths_1.nicePath)(path),
				),
			);
			return;
		...
		if (!(0, validation_1.validate)(tsDocSchema, config)) {
			logger.error(
				logger.i18n.invalid_tsdoc_json_0((0, paths_1.nicePath)(path)),
			);
			return;
		}
		const workingConfig = {};

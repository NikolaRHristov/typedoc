...
class TypeDocReader {
	...
	async read(container, logger, cwd) {
		...
		if (!file) {
			if (container.isSet("options")) {
				logger.error(
					logger.i18n.options_file_0_does_not_exist(
	...
	async readFile(file, container, logger, seen) {
		...
		if (file.endsWith(".json") || file.endsWith(".jsonc")) {
			const readResult = typescript_1.default.readConfigFile(
				(0, paths_1.normalizePath)(file),
				(path) => FS.readFileSync(path, "utf-8"),
			);
			if (readResult.error) {
				logger.error(
					logger.i18n.failed_read_options_file_0(
						(0, paths_1.nicePath)(file),
					),
				);
				return;
			...
			try {
				try {
					...
					if (error?.code === "ERR_REQUIRE_ESM") {
						...
						const esmPath = (0, url_1.pathToFileURL)(
							file,
						).toString();
		...
		for (const [key, val] of Object.entries(data)) {
			try {
				container.setValue(
					key,
					val,
					(0, path_1.resolve)((0, path_1.dirname)(file)),
				);
			} catch (error) {

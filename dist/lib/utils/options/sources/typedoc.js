...
function addTypeDocOptions(options) {
	...
	options.addDeclaration({
		...
		validate(value, i18n) {
			...
			const valid = new Set(
				(0, enum_1.getEnumKeys)(kind_1.ReflectionKind),
			);
			...
			if (!(0, highlighter_1.getSupportedThemes)().includes(value)) {
				throw new Error(
					i18n.highlight_theme_0_must_be_one_of_1(
						"lightHighlightTheme",
						(0, highlighter_1.getSupportedThemes)().join(", "),
					),
				);
			}
		},
	...
	options.addDeclaration({
		...
		validate(value, i18n) {
			const invalid = (0, set_1.setDifference)(
				value,
			...
			if (invalid.size !== 0) {
				throw new Error(
					i18n.option_0_specified_1_but_only_2_is_valid(
						`kindSortOrder`,
						Array.from(invalid).join(", "),
						(0, enum_1.getEnumKeys)(kind_1.ReflectionKind).join(
							", ",
						),
					),
				);

...
function loadPackageManifest(logger, packageJsonPath) {
	...
	if (typeof packageJson !== "object" || !packageJson) {
		logger.error(
			logger.i18n.file_0_not_an_object(
				(0, paths_1.nicePath)(packageJsonPath),
			),
		);
		return undefined;
	}
...
function expandPackages(logger, packageJsonDir, workspaces, exclude) {
	...
	return workspaces.flatMap((workspace) => {
		const expandedPackageJsonPaths = (0, fs_1.glob)(
			(0, path_1.resolve)(packageJsonDir, workspace, "package.json"),
			(0, path_1.resolve)(packageJsonDir),
		);
		if (expandedPackageJsonPaths.length === 0) {
			logger.warn(
				logger.i18n.entry_point_0_did_not_match_any_packages(
					(0, paths_1.nicePath)(workspace),
				),
			);
		} else {
			logger.verbose(
				`Expanded ${(0, paths_1.nicePath)(workspace)} to:\n\t${expandedPackageJsonPaths

...
function deriveRootDir(globPaths) {
	...
	const rootPaths = globs.flatMap((glob, i) =>
		(0, array_1.filterMap)(glob.set, (set) => {
...
function writeFileSync(fileName, data) {
	fs.mkdirSync((0, path_1.dirname)((0, paths_1.normalizePath)(fileName)), {
		recursive: true,
	});
...
async function writeFile(fileName, data) {
	await fs_1.promises.mkdir(
		(0, path_1.dirname)((0, paths_1.normalizePath)(fileName)),
		{
			recursive: true,
		},
	);
...
async function copy(src, dest) {
	...
	if (stat.isDirectory()) {
		...
		await Promise.all(
			contained.map((file) =>
				copy((0, path_1.join)(src, file), (0, path_1.join)(dest, file)),
			),
		);
	} else if (stat.isFile()) {
		await fs_1.promises.mkdir((0, path_1.dirname)(dest), {
			recursive: true,
		});
...
function copySync(src, dest) {
	...
	if (stat.isDirectory()) {
		...
		contained.forEach((file) =>
			copySync((0, path_1.join)(src, file), (0, path_1.join)(dest, file)),
		);
	} else if (stat.isFile()) {
...
function discoverInParentDir(name, dir, read) {
	...
	const reachedTopDirectory = (dirName) =>
		dirName === (0, path_1.resolve)((0, path_1.join)(dirName, ".."));
	...
	return discoverInParentDirExactMatch("package.json", dir, (content) => {
		...
		if (
			(0, validation_1.validate)(
				{ name: String, version: (0, validation_1.optional)(String) },
				pkg,
			)
		) {
			return pkg;

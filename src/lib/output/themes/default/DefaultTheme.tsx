...
import { UrlMapping, type RenderTemplate } from "../../models/UrlMapping";
...
export class DefaultTheme extends Theme {
	...
	getRenderContext(pageEvent: PageEvent<Reflection>) {
		return new DefaultThemeRenderContext(
			this,
			pageEvent,
	...
	buildNavigation(project: ProjectReflection): NavigationElement[] {
		...
		function toNavigation(
			...
			if (
				element instanceof ReflectionCategory ||
				element instanceof ReflectionGroup
			) {
				if (!children?.length) {
					return;
				}
			...
			if (
				...
				element instanceof ReferenceReflection
			) {
				return;
			}
		...
		function filterMapWithNoneCollection(
			reflection: ReflectionGroup[] | ReflectionCategory[],
		) {
			...
			if (none) {
				const noneMappedChildren = filterMap(
					none.children,
					toNavigation,
				);

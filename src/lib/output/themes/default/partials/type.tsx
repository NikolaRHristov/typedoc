...
function includeIndentation(): JSX.Element {
	return indentationDepth > 0 ? (
		<span>{"\u00A0".repeat(indentationDepth * indentSize)}</span>
	) : (
		<></>
	);
}
	...
	conditional(context, type) {
		...
		const parts: JSX.Element[] = [
			...
			renderType(
				context,
				type.extendsType,
				TypeContext.conditionalExtends,
			),
			<br />,
			includeIndentation(),
			<span class="tsd-signature-symbol">? </span>,
				...
				{type.tail.map((item) => (
					<>
						...
						{renderType(
							context,
							item[0],
							TypeContext.templateLiteralElement,
						)}
						<span class="tsd-signature-symbol">{"}"}</span>
						{item[1] && (

...
describe("Internationalization", () => {
    it("Does not include strings in translatable object which are unused", () => {
        const options = new __1.Options(new internationalization_1.Internationalization(null).proxy);
        const tsconfigReader = new __1.TSConfigReader();
        tsconfigReader.read(options, new __1.Logger(), process.cwd());
        const translatableTs = (0, path_1.join)(__dirname, "../../lib/internationalization/translatable.ts");
        const host = {
            ...
            getScriptSnapshot: (fileName) => {
                if (!(0, fs_1.existsSync)(fileName))
                    return undefined;
                return typescript_1.default.ScriptSnapshot.fromString((0, fs_1.readFileSync)(fileName, "utf-8"));
            },
            getCurrentDirectory: () => process.cwd(),
            getCompilationSettings: () => options.getCompilerOptions(),
            getDefaultLibFileName: (opts) => typescript_1.default.getDefaultLibFilePath(opts),
        ...
        (0, strict_1.ok)(typescript_1.default.isVariableDeclaration(translatable.valueDeclaration));
        (0, strict_1.ok)(typescript_1.default.isAsExpression(translatable.valueDeclaration.initializer));
        (0, strict_1.ok)(typescript_1.default.isObjectLiteralExpression(translatable.valueDeclaration.initializer.expression));
        const translatableObj = translatable.valueDeclaration.initializer.expression;
        translatableObj.forEachChild((child) => {
            (0, strict_1.ok)(typescript_1.default.isPropertyAssignment(child));
            const refs = service.getReferencesAtPosition(sf.fileName, child.getStart());
            const refCount = refs?.filter((ref) => !/locales\/.*\.cts$/.test(ref.fileName) &&
                !ref.fileName.endsWith("translatable.ts")).length ?? 0;
            (0, strict_1.ok)(refCount, `Translatable key ${child.name.getText()} is not referenced.`);
        });
        service.dispose();
    });
});

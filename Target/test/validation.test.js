...
function expectWarning(typeName, file, referencingName, intentionallyNotExported = []) {
    ...
    logger.expectMessage(`warn: ${typeName}, defined in */${file}, is referenced by ${referencingName} but not included in the documentation`);
}
...
describe("validateExports", () => {
    ...
    it("Should warn if intentionallyNotExported contains unused values", () => {
        ...
        logger.expectMessage("warn: The following symbols were marked as intentionally not exported, but were either not referenced in the documentation, or were exported:\n\tnotDefined");
        logger.expectNoOtherMessages();
    });
});
describe("validateDocumentation", () => {
    it("Should correctly handle functions", () => {
        ...
        const logger = new TestLogger_1.TestLogger();
        (0, documentation_1.validateDocumentation)(project, logger, ["Function"]);
        logger.expectMessage("warn: bar (CallSignature), defined in */function.ts, does not have any documentation");
        logger.expectNoOtherMessages();
    });
    it("Should correctly handle accessors", () => {
        const project = convertValidationFile("getSignature.ts");
        const logger = new TestLogger_1.TestLogger();
        (0, documentation_1.validateDocumentation)(project, logger, ["Accessor"]);
        logger.expectMessage("warn: Foo.foo (GetSignature), defined in */getSignature.ts, does not have any documentation");
        logger.expectNoOtherMessages();
    });
    it("Should correctly handle constructors", () => {
        const project = convertValidationFile("class.ts");
        const logger = new TestLogger_1.TestLogger();
        (0, documentation_1.validateDocumentation)(project, logger, ["Constructor"]);
        logger.expectMessage("warn: Foo.constructor (ConstructorSignature), defined in */class.ts, does not have any documentation");
        logger.expectNoOtherMessages();
    });
    it("Should correctly handle interfaces", () => {
        ...
        logger.expectMessage("warn: Foo.method (CallSignature), defined in */interface.ts, does not have any documentation");
        logger.expectNoOtherMessages();
    });
    it("Should correctly handle callback parameters", () => {
        const project = convertValidationFile("callbackParameters.ts");
        const logger = new TestLogger_1.TestLogger();
        (0, documentation_1.validateDocumentation)(project, logger, ["Parameter", "Property"]);
        logger.expectNoOtherMessages();
    });
});

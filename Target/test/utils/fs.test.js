...
describe("fs.ts", () => {
    describe("getCommonDirectory", () => {
        ...
        it("Returns the dirname if only one file is provided", () => {
            (0, assert_1.deepStrictEqual)((0, fs_1.getCommonDirectory)(["a/b/c.ts"]), "a/b");
        });
        it("Handles duplicates paths appropriately", () => {
            const p = "a/b/c";
            (0, assert_1.deepStrictEqual)((0, fs_1.getCommonDirectory)([p, p]), p);
        });
        it("Gets the path common to all files", () => {
            (0, assert_1.deepStrictEqual)((0, fs_1.getCommonDirectory)(["/a/b/c", "/a/b/c/d/e", "/a/b/d"]), "/a/b");
        });
    });
    describe("glob", () => {
        let fix;
        ...
        it("handles root match", () => {
            fix.write();
            const result = (0, fs_1.glob)(fix.cwd, fix.cwd, { includeDirectories: true });
            (0, assert_1.deepStrictEqual)(result.map(path_1.normalize), [fix.cwd].map(path_1.normalize));
        });
        it("Handles basic globbing", () => {
            ...
            fix.addFile("b.js");
            fix.write();
            (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`${fix.cwd}/*.ts`, fix.cwd).map((f) => (0, path_1.basename)(f)), ["a.ts", "test.ts", "test2.ts"]);
            (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`**/test*.ts`, fix.cwd).map((f) => (0, path_1.basename)(f)), ["test.ts", "test2.ts"]);
        });
        describe("when 'followSymlinks' option is true", () => {
            it("should navigate symlinked directories", () => {
                const target = (0, path_1.dirname)(fix.dir("a").addFile("test.ts").path);
                fix.write();
                fs.symlinkSync(target, (0, path_1.resolve)(fix.cwd, "b"), "junction");
                (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`${fix.cwd}/b/*.ts`, fix.cwd, {
                    followSymlinks: true,
                }).map((f) => (0, path_1.basename)(f)), ["test.ts"]);
            });
            it("should navigate recursive symlinked directories only once", () => {
                fix.addFile("test.ts");
                fix.write();
                fs.symlinkSync(fix.cwd, (0, path_1.resolve)(fix.cwd, "recursive"), "junction");
                (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`${fix.cwd}/**/*.ts`, fix.cwd, {
                    followSymlinks: true,
                }).map((f) => (0, path_1.basename)(f)), ["test.ts", "test.ts"]);
            });
            it("should handle symlinked files", function () {
                const { path } = fix.addFile("test.ts");
                fix.write();
                try {
                    fs.symlinkSync(path, (0, path_1.resolve)((0, path_1.dirname)(path), "test-2.ts"), "file");
                }
                catch (err) {
                ...
                (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`${fix.cwd}/**/*.ts`, fix.cwd, {
                    followSymlinks: true,
                }).map((f) => (0, path_1.basename)(f)), ["test-2.ts", "test.ts"]);
            });
        });
        describe("when node_modules is present in the pattern", function () {
            it("should traverse node_modules", function () {
                ...
                fix.write();
                (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`${fix.cwd}/node_modules/test.ts`, fix.cwd).map((f) => (0, path_1.basename)(f)), ["test.ts"]);
            });
        });
        describe("when node_modules is not present in the pattern", function () {
            it("should not traverse node_modules", function () {
                fix.dir("node_modules").addFile("test.ts");
                fix.write();
                (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`${fix.cwd}/**/test.ts`, fix.cwd).map((f) => (0, path_1.basename)(f)), []);
            });
        });
        it("should ignore anything that is not a file, symbolic link, or directory", function (done) {
            ...
            const sockServer = (0, net_1.createServer)()
                ...
                let err = null;
                try {
                    (0, assert_1.deepStrictEqual)((0, fs_1.glob)(`${fix.cwd}/*.sock`, fix.cwd), []);
                }
                catch (e) {
                    err = e;
                }
